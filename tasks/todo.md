...
| 2 | Broader Metal GPU path (more kernels / reduce) | ◑ Improved | Fused cosine/dot/L2 + elementwise add/sub/max/min/div + unary scale/relu/abs/negate + binary mul + multi-pass reduce + demo softmax. Added public `VectorOps.{abs,negate,mul,reduceSum}` / `GpuCompute.reduceSum` + Metal kernels (parity tests). Residual: more kernels / CUDA/ANE disclosed. |
...
| 5 | Phase D cutover plan (HITL) | ◑ Strangler evidence landed | Phases 0–4 extracts + MCP audit on PR #743 (`cursor/strangler-redesign-phases`). Evidence: `docs/spec/phase-d-cutover-plan.mdx` §Strangler evidence. Gate 6 HITL still open — **keep `src/` build root**; no second `build.zig`. Scaffold honesty: `tools/check_modernized_refs.sh` in `full-check`. |
...
- **Strangler redesign phases 0–4** — foundation `http/` leaves, `wdbx/cluster_policy`, `ai/browser_plan`, MCP isolation audit; evidence in `docs/spec/phase-d-cutover-plan.mdx` §Strangler evidence (PR #743). Gate 6 HITL still open; `src/` remains build root.
- **Phase 0 redesign baseline** — `./build.sh check` green 2026-07-23 on pin `0.17.0-dev.1442+972627084`; evidence in `docs/spec/phase-d-cutover-plan.mdx` §Current state. Cutover still HITL-gated.
...
- **Public `reduceSum`** — `GpuCompute.reduceSum` / `VectorOps.reduceSum` expose the existing Metal multi-pass `reduce_sum_kernel` (`runReduceSum`) through the public API, mirroring `reduceMax`/`reduceMin` (0 for empty input, CPU fallback, CPU/GPU parity tests). Source: src/features/gpu/{compute_api,vector_ops,stub}.zig, tests/contracts/feature_modules.zig, docs/contracts/external-claims-audit.mdx, CHANGELOG.md.
...
- **Broader Metal + docs/contracts slice** — public `VectorOps.{mul,negate}` + Metal kernels (reusing dot/negate pipelines), parity tests, contract @hasDecl coverage; docs sync (external-claims, public-api, north-star) for abs/negate/mul + keychain/cluster disclosures; scoped local_bridge skip/owned test; modernized + docs-validate green. All via scoped plan slices.
