...
| 2 | Broader Metal GPU path (more kernels / reduce) | ◑ Improved | Fused cosine/dot/L2 + elementwise add/sub/max/min/div + unary scale/relu/abs/negate + binary mul + multi-pass reduce + demo softmax. Added public `VectorOps.{abs,negate,mul}` + Metal kernels (parity tests). Residual: more kernels / CUDA/ANE disclosed. |
