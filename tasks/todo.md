...
| Codex/claude-code TUI feature parity | ◑ Partial | Landed: raw-mode line editor, slash-commands (`/open`, `/diff`, `/commit`, `/context`, `/features`, `/learn`, `/live`, `/save`/`/load`, `/sessions`, `/clear`, `/pane`), plugin commands + context providers, multi-turn ring history, session save/load, Ctrl-R reverse search, Alt-Enter multi-line, Emacs keys, colorized `/diff`, unified context budgets, post-turn `/pane` split, live Anthropic SSE via `/live`. Remaining: true token-by-token in-process generation (template model). No new top-level CLI commands (frozen surface). |
| Streaming token-by-token completion | ◑ Partial (improved) | Local persona path: post-hoc ~4-byte chunks via `stream_callback`. Local-bridge: OpenAI-compatible SSE. Live Anthropic: Messages SSE on `complete --live --stream` and TUI `/live`. Residual: true incremental in-process generation. |
...
| Zero-copy RankedNode vector views | ⚪ Not started | CLI/JSON already borrow `getVector` for dims after search; residual is threading borrowed slices through `SearchResult`/`RankedNode` without dangling after unlock/mutation. |
...
- **`abi wdbx query` UX** — `--limit`/`--json`/`--text`/`--persona` (legacy positionals kept); JSON shape mirrors hybrid ranking components + borrowed `dims`/`vector_view`; post-search `getVector` zero-copy alias test. Residual: RankedNode still id/score-only.
...
- **modern-refactor Phase 1** — filled advertised skill `references/` (`analysis-checklist.md`, `implementation-playbook.md` + example); layout verifier; `.gitignore` allowlist + honest README; docs archive isolation + standard extract disclaimers; `tools/goal_capture.sh` SCRATCH via env/`TMPDIR`. (Phases 2–4 completed in git history; product reimagine stays separate/HITL-blocked.)
...
- **Instruction files compacted + reconciled** — AGENTS.md rewritten to ~70 lines (high-signal only: toolchain, commands, feature flags, architecture, frozen surfaces, parity, Zig 0.17 patterns, claims discipline, CI, Linux caveat); CLAUDE.md and GEMINI.md re-synced to the same compact form so the three siblings no longer drift. `check-parity` + `public_docs` contract still green.
...
- **Cross-compilation CI** — `.github/workflows/ci.yml` runs `zig build check` + `zig build cross-smoke` across linux-gnu/windows-gnu/aarch64-macos compile/link targets; Windows runtime execution remains an open verification item above. **Linux libc-link + objc gating landed**: exe/`abi-mcp` + every test module sets `link_libc=true`; `metal_shared.zig` gates objc externs to macOS via `comptime`. Ambient WDBX persist EBADF on Linux is fixed (`iterate=true` dir open). Cross-link verified for linux-gnu; execution still needs a Linux host / CI `cross-smoke`.
...
- **Instruction files: thin redirects** — CLAUDE.md/GEMINI.md compressed from ~50→5 lines each (thin redirects to AGENTS.md); AGENTS.md preamble updated: update only this file, not three siblings. Deleted 4 stale merged branches — only `main` remains. `./build.sh check` 39/39. (#671)
