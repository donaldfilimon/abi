...
| Scratch-path hardening for the os-control audit tests | ✅ | 2026-08-01: the two tests built their scratch dir from `{pid}-{thread_id:?}`; both now use `abi_foundation::temp_path::temp_file_path()` (PID + per-process counter), which cannot collide with anything this process made earlier. Good hygiene, kept on its own merits. |
| CI flake: `os::audit`/`os` scratch-store `WriterBusy` | ✅ | Fixed in [#772](https://github.com/donaldfilimon/abi/pull/772) by the writer-lock retry, **not** by the row above. Measured head-to-head on the same harness (`os::` at `--test-threads 8`, 40 runs each): scratch-path hardening alone still failed **3/40**; the writer-lock retry alone and both together failed **0/40**. The original "leftover dir from a panicked run" theory is not reachable — a dead process cannot hold an `flock` (the kernel releases it at exit) and `scratch_store` already did `remove_dir_all` first. The real holder is live and transient: a `fork` duplicates the lock's fd into the child until `exec` closes it via O_CLOEXEC. |
