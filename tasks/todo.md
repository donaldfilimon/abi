...
| 5 | Phase D cutover plan (HITL) | ◑ Strangler in progress | Clean-slate = Approach-1 extracts inside `src/` along `modernized/packages/*` DAG (no second `build.zig`). Plan: Cursor `abi_clean-slate_redesign`. Evidence checklist: `docs/spec/phase-d-cutover-plan.mdx`. Scaffold honesty: `tools/check_modernized_refs.sh` in `full-check`. Cutover still needs explicit HITL — do not claim complete. |
