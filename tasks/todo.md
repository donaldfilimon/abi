...
- **Credential-file hardening** — `abi auth` now creates/repairs `~/.abi` as owner-only (`0700`) and opens/truncates `credentials.json` as owner-only (`0600`) before writing secrets on POSIX-capable targets; still plaintext. Secret heap/stdin buffers are `secureZero`'d after use; keychain/Windows ACL remain disclosed future work.
