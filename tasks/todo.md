...
| Codex/claude-code TUI feature parity | ◑ Partial | Landed: raw-mode line editor, slash-commands (`/open`, `/diff`, `/commit`, `/context`, `/features`, `/learn`, `/save`/`/load`, `/sessions`, `/clear`), plugin commands + context providers, multi-turn ring history (`MAX_TURN_HISTORY`) injected into completions, session save/load in `repl_session.zig`, Ctrl-R reverse search, Alt-Enter multi-line, Ctrl-K/U/W Emacs keys, colorized `/diff`, `/diff --stat`, unified context budgets. Remaining: pane split (chat + diffs), true token-by-token terminal redraw. No new top-level CLI commands (frozen surface). |
