...
| Codex/claude-code TUI feature parity | ◑ Partial | Landed: raw-mode line editor, slash-commands (`/open`, `/diff`, `/commit`, `/context`, `/features`, `/learn`, `/save`/`/load`), plugin commands + context providers. Remaining: multi-turn context window policy, pane split (chat + diffs), true token-by-token terminal redraw. No new top-level CLI commands (frozen surface). |
| Streaming token-by-token completion | ◑ Partial | `completeStreaming` / `stream_callback` (~16-byte post-hoc chunks) + local SSE server in `streaming.zig`. Not end-to-end token streaming in `agent tui` from a chunked model backend. Design: bridge SSE → `line_editor` incremental append. |
