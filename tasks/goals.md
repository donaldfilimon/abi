...
> **Current path note (2026-08-27):** the operational checkout is
> `/Users/donaldfilimon/dev/active/abi`. The substrate crates
> `abi-foundation`, `abi-core`, `abi-telemetry`, `abi-compute`, and `abi-wdbx`
> now live in the required sibling `../wdbx/crates/`. Older `~/abi`,
> `/Users/donaldfilimon/abi`, and ABI-local substrate paths below are retained
> only as dated historical evidence; do not use them as current commands.
...
- **Fix 1 — spurious `WriterBusy` (now at `../wdbx/crates/abi-wdbx/src/durable.rs`).** The advisory writer lock lives on the open file description, so any `fork` in the process duplicates it into the child until `exec` closes it via O_CLOEXEC. A store dropped just before an unrelated `Command::spawn` stays locked for the width of that window. Not test-only: `abi agent os execute` holds the audit store open *while spawning the command it audits*, so a second `abi` invocation could be told the store was busy when no writer owned it. `acquire_writer_lock` now waits out `WouldBlock` for 50 ms in 1 ms steps; a genuinely held lock still reports `WriterBusy`, and a real filesystem error is never retried. Diagnosed empirically: an `lsof` probe at the moment of failure found **no holder**, and a retry probe recovered on the **first** 1 ms attempt in 6/6 caught failures.
...
- **2026-08-19 correction:** the billing lock cleared and PR #794's Windows Server 2025 job genuinely executed. It passed 2 ACL tests and 7 credential-file tests after adding the protected-DACL flag and accepting Windows' `AI` bookkeeping form. The earlier downgrade above remains the historical state when written; Windows ACL runtime coverage is now verified.
...
- Current foundation boundary: `abi-agent-runtime` defines provider-neutral model, event, tool-description, policy, audit, usage, budget, cancellation, and deterministic-fixture contracts. Its read-only `RunContext::checkpoint` lets bounded blocking adapters poll cancellation and budgets without emitting fake events or receiving a cancellation token. It does **not** execute tools or perform model inference.
...
- Local-model execution slice implemented: the additive `abi-model-runtime` crate resolves an exact principal-authorized manifest and loads verified external Tokenizers/Safetensors artifacts into an explicitly requested Candle device. Generated scratch fixtures prove deterministic CPU execution and locally exercised Metal tensor operations. Load and inference reports distinguish requested, initialized, executed, runtime-verified, fallback, and mixed execution; no speed or placement claim is inferred. The only architecture implemented here is the tiny offline `abi-bigram-v1` fixture. Gemma, structured tool-call generation, and committed model data remain future work; CUDA is source-gated but has no local compile/runtime evidence because this host has no `nvcc`.
- Worker-foundation slice implemented: the additive `abi-worker` crate authenticates signed jobs and cancellations against audience-bound coordinator keys, binds identity and namespace, derives principal-scoped idempotency digests inside the manager, issues finite leases, propagates cooperative cancellation, bounds quotas and ordered result streams, verifies result hashes, rejects replay/conflict, and reconstructs append-only job-control state. Its transport configuration reuses the gateway's owner-protected mandatory-mTLS loader, while its control-store trait remains independent from WDBX data-plane authority. It adds no CLI or MCP command and does not claim a listener, scheduler, model executor, certificate operations, production cluster, or separate-host proof.
...
## Fix and improve the GPU/accelerator surface
status: done
- Scope: `crates/abi-gpu` plus the CLI surfaces that render its evidence (`abi backends`, `abi wdbx compute`). Claim-honesty is the acceptance bar: no native-dispatch claim from compilation or availability alone.
- Slice implemented 2026-09-08 (uncommitted in the working tree at `2df93848`): `abi backends` printed the abstraction-only compatibility table under the label `usable=`, so `gpu-cuda` and `gpu-vulkan` both read `usable=true` on an Apple Silicon host with neither runtime present, contradicting `effective=cpu-neon` in the same row. The rows now print as `compat <backend> … service_available=… native=… effective=…`, matching the wording `abi wdbx compute` already used for the identical field, with a comment stating that `service_available` means a request is served (via deterministic CPU fallback) and is not an accelerator-presence claim. `./tools/check.sh` exit 0, `check: all green`. No contract changed.
- Deliberately NOT changed: `abi_compute::capabilities()` sets `available: backend.is_accelerator() || …` on purpose. That is pinned by `compatibility_capabilities_keep_fallback_availability` in `wdbx/crates/abi-compute/src/backend.rs` and consumed as JSON by `abi-cli/src/browser_studio.rs`. Altering the field's meaning is a cross-repo contract decision for Donald, not a rendering fix. The defect was the CLI label.
- Measured runtime evidence on this host (Mac17,2, macOS 27): Metal DOT is linked, initialized, and active — `abi backends` reports `accelerated=yes` with `Native kernels Metal DOT linked+initialized`. CUDA and Vulkan remain compile/target work with no local runtime proof. ANE reports `hardware_present=true native_dispatch=false`; the CoreML surface proves an accepted `.cpuAndNeuralEngine` request plus a matching tiny-model inference, which is not ANE residency and correctly never sets `runtime_verified`.
- Open residual (next slice, evidence-backed): `abi-gpu` runs an identical 20 tests under all five feature configurations measured — default, `--all-features`, `--no-default-features`, `--no-default-features --features metal-kernels`, and `--no-default-features --features cuda-adapter,vulkan-adapter`. No test is `#[cfg(feature)]`-gated; the adapter tests instead branch on `metal_kernels::kernels_active()` at runtime and assert the correct invariant on each side. That is sound, but on a Metal-capable host only the active branch ever executes, so the kernels-inactive degradation path has no local proof. Making the inactive path directly exercisable (an injectable probe, or a cfg-gated test) would close it.
- Open residual: the compat table still prints `native=false` for `gpu-metal` four lines below the authoritative `Metal DOT active` line. The `compat` prefix now scopes the row as the abstraction-only view rather than a runtime claim, so the two no longer contradict, but the underlying duplication of two capability tables across `abi-gpu` and `abi-compute` is unresolved.
- Inactive-path slice implemented 2026-09-08 (uncommitted): `MetalAccelerator` reaches the native kernels through a function-pointer seam. Production uses `MetalKernels::REAL`, so dispatch behaviour is unchanged; a test-only constructor injects linked-but-inactive kernels so the CPU-fallback path is provable on this Metal-capable host, where the pre-existing runtime `if` could only ever take the accelerated branch. The new test asserts the adapter still returns the deterministic CPU oracle for dot/cosine/norm/top_k while `executed` and `runtime_verified` stay false. Verified by mutation, not by a green line: injecting `MetalKernels::REAL` into the inactive constructor makes it fail at `!before.initialized()`, and it passes only when correctly wired. 21 tests (was 20) pass under default, `--all-features`, `--no-default-features`, `+metal-kernels`, and `+cuda-adapter,vulkan-adapter`. `./tools/check.sh` exit 0, `check: all green`.
- Gate caught two defects in that slice before it was reported green: a rustfmt diff, and clippy `type_complexity` on the `top_k` function-pointer field. The latter was fixed with `ActiveFn`/`BinaryFn`/`UnaryFn`/`TopKFn` aliases — clippy's own suggested remedy — rather than an `#[allow]`, since suppressing the lint would have hidden the complaint instead of answering it.
- Still open and unchanged by this slice: the two-capability-table duplication (`abi-gpu` evidence ladder vs `abi-compute` compat table) and the CUDA/Vulkan/ANE evidence boundaries recorded above. Nothing here establishes accelerator speedup, CUDA/Vulkan runtime execution, or ANE residency.

- 2026-09-16 13:5x EDT, `/goal continue with all`, closing slice on Donald's decision: the two capability tables stay separate. `abi backends` drops its five `compat … service_available=` rows (they sat four lines under `Native accelerator kernels: Metal DOT active` and read as contradicting it) and prints `Compatibility table + evidence ladder: \`abi wdbx compute info\`` instead; that command already printed both the compat rows and the per-backend `evidence` ladder, verified on the built binary, so the "print the ladder there" half needed no code. Test in `backends.rs` now pins: no `compat ` / `service_available=` in `abi backends`, pointer line present, still no `native=true`. Hardware-bound residuals are unchanged and stay disclosed Partial, not in this goal's scope: CUDA/Vulkan adapters not compiled on this host, ANE runtime residency not verified. Gate result recorded in the commit.
## Fix rust-objcopy strip aborts in release builds through tools/cargo.sh
status: done
- 2026-09-16 13:4x EDT: root cause was two-layered. rustc on Apple targets always strips with `rust-objcopy` (verified in `link.rs` at `0dfb098f`, the pinned nightly's commit), whose `@rpath/libLLVM.dylib` resolves only via the `DYLD_FALLBACK_LIBRARY_PATH` the rustup proxy injects; `tools/cargo.sh` execs the toolchain cargo directly and dropped it. Fix: export the same fallback list (`<toolchain>/lib:$HOME/lib:/usr/local/lib:/usr/lib`) in the wrapper. Verified: a scratch crate built through the wrapper goes from the warning to 0 warnings; `build --release --locked -p abi-cli -p abi-wdbx-gateway` into a scratch target dir finishes with 0 warnings and both binaries pass `codesign --verify` and execute; `./tools/check.sh` exit 0 after the edit. Not done on purpose: no `llvm-tools` install, no dylib symlink, no other toolchain touched (every other path goes through the proxy). Committed locally; push is Donald's call.
- 2026-09-16 13:4x EDT, correction to the bullet above: a second `./tools/check.sh` at 13:45, run before committing, exited 1 at the `format (check only)` step on `../wdbx/crates/abi-wdbx/tests/v3_cross_language_commitment.rs`, an UNTRACKED file in the sibling repo written at 13:44:52 by another live session (wdbx HEAD `dd73327`, that file `??`). The 13:42 run on the same abi content had 0 rustfmt diffs. The red is that session's in-flight wdbx work, not this wrapper change; wdbx was not touched.
- 2026-09-16 13:5x EDT, `/goal continue`: `ee435e9e` (backends pointer line) and `056beeb4` (skill mirror) PUSHED, `bf778c47..056beeb4` on `origin/main`, 0 ahead after fetch; the "push is Donald's call" wording in the two bullets above is history. Central `/sync-clis` dry run reported `Done. 0 actions/changes.` and every managed copy of `abi-superpower-wdbx-compute/SKILL.md` matches the central source byte-for-byte. The ABI in-repo mirror launcher's dry run wants to write `abi/zig-types` and `abi/zig` skill mirrors; that is pre-existing drift the sync skill's own invariant forbids reintroducing, so it was NOT applied and is not this goal's residual.
