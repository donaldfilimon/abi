...
pub fn main(_: std.process.Init) !void {
    ...
    // Start event
    const start_event = abi.features.ai.streaming.StreamEvent.startEvent();
    std.debug.print("  Start event type: {t}\n", .{start_event.event_type});

    // Token events
    const tokens = [_][]const u8{ "Hello", ",", " ", "world", "!" };
    for (tokens, 0..) |text, i| {
        const token = abi.features.ai.streaming.StreamToken{
            .id = @intCast(i),
            .text = text,
            .is_end = (i == tokens.len - 1),
            .sequence_index = i,
        };
        const event = abi.features.ai.streaming.StreamEvent.tokenEvent(token);
    ...
    // End event
    const end_event = abi.features.ai.streaming.StreamEvent.endEvent();
    std.debug.print("  End event type: {t}\n", .{end_event.event_type});

    // Error event
    const error_event = abi.features.ai.streaming.StreamEvent.errorEvent("Connection timeout");
    std.debug.print("  Error event: {s}\n", .{error_event.error_message orelse "none"});

    // Heartbeat event
    const heartbeat = abi.features.ai.streaming.StreamEvent.heartbeatEvent();
    std.debug.print("  Heartbeat event type: {t}\n\n", .{heartbeat.event_type});

    // SSE Encoding
    std.debug.print("SSE Encoding:\n", .{});
    var encoder = abi.features.ai.streaming.sse.SseEncoder.init(allocator, .{
        .include_timestamp = true,
        .include_id = true,
    });

    const sample_token = abi.features.ai.streaming.StreamToken{
        .id = 42,
        .text = "example",
        .is_end = false,
        .sequence_index = 0,
    };
    const sample_event = abi.features.ai.streaming.StreamEvent.tokenEvent(sample_token);
    ...
    const event_types = [_]abi.features.ai.streaming.StreamEventType{
        .token,
        .start,
        .end,
        .error_event,
    ...
    const backend_types = [_]abi.features.ai.streaming.BackendType{
        .local,
        .openai,
        .ollama,
        .anthropic,
