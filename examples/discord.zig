...
pub fn main() !void {
    ...
    var client = abi.discord.createClient(allocator) catch |err| {
        switch (err) {
            abi.discord.DiscordError.MissingBotToken => {
    ...
    if (intents & abi.discord.GatewayIntent.GUILDS != 0)
        std.debug.print("  - GUILDS\n", .{});
    if (intents & abi.discord.GatewayIntent.GUILD_MESSAGES != 0)
        std.debug.print("  - GUILD_MESSAGES\n", .{});
    if (intents & abi.discord.GatewayIntent.DIRECT_MESSAGES != 0)
        std.debug.print("  - DIRECT_MESSAGES\n", .{});
    if (intents & abi.discord.GatewayIntent.MESSAGE_CONTENT != 0)
        std.debug.print("  - MESSAGE_CONTENT (privileged)\n", .{});
    if (intents & abi.discord.GatewayIntent.GUILD_MEMBERS != 0)
        std.debug.print("  - GUILD_MEMBERS (privileged)\n", .{});
    if (intents & abi.discord.GatewayIntent.GUILD_PRESENCES != 0)
