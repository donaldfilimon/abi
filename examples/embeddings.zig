...
pub fn main(_: std.process.Init) !void {
    ...
    // L2 norms using SIMD
    const norm_a = abi.foundation.simd.vectorL2Norm(vec_a);
    const norm_b = abi.foundation.simd.vectorL2Norm(vec_b);
    std.debug.print("L2 Norm of vec_a: {d:.6}\n", .{norm_a});
    std.debug.print("L2 Norm of vec_b: {d:.6}\n\n", .{norm_b});

    // Cosine similarity using SIMD
    const similarity = abi.foundation.simd.cosineSimilarity(vec_a, vec_b);
    ...
    for (0..num_vectors) |i| {
        const sim = abi.foundation.simd.cosineSimilarity(query, vectors[i]);
