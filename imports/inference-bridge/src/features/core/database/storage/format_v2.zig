//! WDBX Storage Format v2 — legacy on-disk types kept for migration reads.
...
pub const FileHeader = struct {
    ...
    pub fn serialize(self: FileHeader) [64]u8 {
        ...
        @memcpy(buf[HEADER_UUID_OFFSET .. HEADER_UUID_OFFSET + HEADER_UUID_SIZE], &self.uuid);
        @memcpy(
...
pub const FileFooter = struct {
    ...
    pub fn deserialize(buf: [32]u8) !FileFooter {
        if (!std.mem.eql(u8, buf[0..4], &.{ 'E', 'N', 'D', 0 })) return error.InvalidFooter;
        return .{
            .marker = buf[0..4].*,
