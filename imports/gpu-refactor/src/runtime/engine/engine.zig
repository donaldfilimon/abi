...
const Condition = struct {
    ...
    pub fn timedWait(self: *Condition, mutex: anytype, timeout_ns: u64) error{Timeout}!void {
        _ = self.waiters.fetchAdd(1, .monotonic);
        mutex.unlock();
        var elapsed: u64 = 0;
        const spin_ns: u64 = 1_000; // ~1us per spin iteration estimate
        while (self.waiters.load(.acquire) > 0) {
...
fn createTaskNode(
    ...
    const NodeOps = struct {
        fn run(allocator: std.mem.Allocator, context: *anyopaque) TaskExecuteError!ResultBlob {
            ...
            // encodeResult duplicates byte slices, so free the original to avoid leaks.
            defer if (comptime isByteSlice(ResultType)) {
