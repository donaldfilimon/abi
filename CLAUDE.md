...
Trust executable config over prose: when this file conflicts with `build.zig`, `tools/build.sh`, or source, trust the executable source. `AGENTS.md` is the canonical instruction file for this repo; `tasks/lessons.md` is the session-start checklist, `tasks/todo.md` tracks active work, and `tasks/goals.md` holds long-horizon direction.
