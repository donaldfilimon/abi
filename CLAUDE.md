...
- **WDBX Cluster Claims**: `cluster serve` is a real TCP RequestVote/AppendEntries listener; `ABI_WDBX_CLUSTER_TOKEN` is required for non-loopback binds and `ABI_WDBX_CLUSTER_PEERS` can allowlist node ids. Do not present this as production multi-host deployment or sharding.
