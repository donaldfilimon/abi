...
- GPU/modules: all feature modules keep real/stub parity; on macOS, Metal fused cosine/dot/L2 parts, elementwise `add`/`sub`/`max`/`min` (`compute_api.map`), multi-pass `reduce_sum_kernel` (256-wide until one scalar), and demo-grade softmax (`softmax_kernel` + `softmax_norm_kernel`) run when native kernels initialize; otherwise GPU status and vector operations fall back deterministically to vectorized CPU. Broader kernels, CUDA/Vulkan, and ANE remain non-claims.
