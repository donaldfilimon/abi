...
## Quick Start
```bash
zig version             # Confirm the pinned Zig 0.17.0-dev.1398+cb5635714 toolchain (see .zigversion; build.zig.zon minimum 0.17.0-dev.1252+e4b325c19)
...
- GPU/modules: all feature modules keep real/stub parity; on macOS, Metal fused cosine/dot/L2 parts, multi-pass `reduce_sum_kernel` (256-wide until one scalar), and demo-grade softmax (`softmax_kernel` + `softmax_norm_kernel`) run when native kernels initialize; otherwise GPU status and vector operations fall back deterministically to vectorized CPU. Broader kernels, CUDA/Vulkan, and ANE remain non-claims.
