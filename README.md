...
- WDBX: contract coverage verifies ordered vector search results, block metadata round-tripping, segment/WAL recovery, temporal graph snapshot restore, and MCP hybrid ranking while disabled builds return explicit `error.FeatureDisabled` operations for key-value/vector/search/block/spatial/temporal writes.
