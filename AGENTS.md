...
- Metal `vectorOps` on macOS: fused cosine/dot/L2, elementwise add/sub/max/min/div via `compute_api`, unary scale/relu, multi-pass `reduce_sum_kernel` / `reduce_max_kernel`, and demo-grade two-pass softmax (`softmax_kernel` + `softmax_norm_kernel`, on-GPU reduce_max when available; host partition-sum — not a perf claim); CUDA/Vulkan stay disclosed stubs (no native dispatch / ANE claimed).
- Same-repo CI `docs-validate` (mint) runs on the self-hosted macOS ARM64 runner; fork PRs use hosted `docs-validate-hosted` on `ubuntu-latest` (untrusted heads never touch self-hosted; hosted runners can be blocked by GitHub billing lock). Windows credential writes can apply owner-only DACL (SDDL `OW`); Windows ACL **runtime** verification needs a Windows host/CI. macOS login keychain is opt-in Partial (`ABI_CREDENTIALS_BACKEND=keychain`); Windows/Linux OS secret stores remain Proposed.
