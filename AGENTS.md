...
- `agent` supports `plan`, `train`, `tui`, `multi`, `spawn`, `browser`, `os`:
  ...
  - `os` runs commands through the `os_control` policy gate: `agent os dry-run <cmd>` applies the same validation and only renders an escaped plan; `agent os execute --confirm <cmd>` is the only execution path, requires the literal `--confirm` token, and maps the read-only allow-list (`true`/`pwd`/`ls`/`whoami`/`date`) to trusted absolute executables. Both modes authorize canonical workspace/cwd paths from opened directory handles; `ls` accepts approved read-only options only and no filesystem operands. Shells (`sh`/`bash`/`zsh`/`fish`), unsafe arguments, and the destructive deny-list remain blocked.
  - `agent tui` REPL slash-commands include `/open <path>` (load file into context), `/diff` (git diff), `/commit` (git commit), `/context` (show context state), `/features` (show build-time feature flags), `/learn` (toggle SEA self-learning mode), `/live` (toggle live Anthropic SSE), `/save <name>` / `/load <name>` (session save/restore), `/sessions` (list saved sessions), `/clear` (clear screen), `/pane` (chat + git diff --stat split), and plugin-provided commands declared via `abi-plugin.json` `commands` field.
...
- **Streaming + file context**: `CompletionRequest`/`LearnLoopConfig` accept `stream_callback`/`stream_ctx` for post-hoc chunked output (~4-byte splits; true per-token streaming requires a chunked model backend). Live Anthropic Messages SSE is wired via `streamMessageLiveIncremental` for `complete --live --stream` and TUI `/live`. `file_context.zig` resolves `@file` mentions (sandboxed to cwd, rejects `..`/absolute/symlink escape) with fair-share budgets (open > @file > tree > git), optional workspace tree listing, and budgeted git diff/--stat for `agent plan`/`multi`.
...
- **Ambient WDBX persistence on Linux:** durable parent dirs are created/repaired owner-only (`0700`) via `ensureOwnerOnlyDir` with `iterate=true` (non-O_PATH) so `fchmod` works. Prefer a scratch `ABI_WDBX_PATH` in tests; `ABI_WDBX_PERSIST=0` remains available for forced in-memory.
...
- Modern-refactor Phases 2–4 and major module-extraction waves are done; `tasks/todo.md` treats post-extraction health as excellent with no high-risk org slices. `dashboard_render.zig` landed (`dashboard.zig` ~399 + render leaf); TUI hub is `repl.zig` (~564) with leaves `repl_commands.zig`, `repl_git_helpers.zig`, `repl_git_commands.zig`, `repl_complete.zig`, `repl_pane.zig`, `repl_session.zig`.
...
- Open product goal for TUI/CLI north-star (streaming, richer `@file`) is in `tasks/goals.md`; `/pane` split has landed. Partial north-star / demo modules must not be promoted to Current without source and tests.
...
- Canonical refactor layout/status: `docs/spec/abi-refactor-design.mdx`; Approach-1 waves A–C complete including `/pane` (`docs/superpowers/plans/2026-07-15-approach1-waves-a-b-c.md`); `modern-refactor/examples/` is historical, not the active board.
...
- Ambient durable-store owner-only repair opens dirs with `iterate=true` (non-O_PATH) so Linux `fchmod` works; `ABI_WDBX_PERSIST=0` is no longer required to avoid EBADF panics on Linux ambient open.
