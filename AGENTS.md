...
- New default-off feature flags: `feat-sea` (`src/features/sea/`, Sparse Evidence Attention self-learning) and `feat-foundationmodels` (`src/connectors/fm.zig`, Apple on-device FoundationModels — macOS-only, links `FoundationModels.framework` + a `swiftc`-built `libabi_fm_shim.dylib`; on-device generation works via a Swift `@c` shim (SE-0495) and is runtime-verified on Apple-Intelligence hardware). Model id `apple-fm` (`fm-local`/`fm`) routes to provider `fm`.
