...
- Metal `vectorOps` on macOS: fused cosine/dot/L2, elementwise add/sub/max/min via `compute_api`, multi-pass `reduce_sum_kernel`, and demo-grade two-pass softmax (`softmax_kernel` + `softmax_norm_kernel`, host-side max/partition-sum — not a perf claim); CUDA/Vulkan stay disclosed stubs (no native dispatch / ANE claimed).
