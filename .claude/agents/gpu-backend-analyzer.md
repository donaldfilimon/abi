...
- Vector ops must produce identical results across backends (determinism); HNSW
  cosine routing (`../wdbx/crates/abi-wdbx/src/hnsw.rs`) depends on this parity.
- Accelerator selection lives in `crates/abi-gpu/` (and WDBX compute dispatch
  in `../wdbx/crates/abi-wdbx/src/compute.rs`) — picks per workload
  (training/inference) with CPU fallback.
