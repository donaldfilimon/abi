...
description: "Analyze abi's AI profile routing and constitution — Abbey/Aviva/Abi router weights, keyword heuristics, constitution principles, and EMA weight persistence. Use when working on `src/features/ai/router.zig` or `src/features/ai/constitution.zig`, or tuning profile selection. Read-only."
---
...
Method: read `src/features/ai/router.zig`, `src/features/ai/constitution.zig`, and the routing path in `src/features/ai/mod.zig`; trace how an input maps to a profile, how weights adapt and persist (EMA), and how the constitution audit gates a completion. Run `abi complete "<text>"` to capture the `profile=`/`audit_passed=` signal.
