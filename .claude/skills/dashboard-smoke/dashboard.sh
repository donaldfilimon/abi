...
# stderr (capture 2>&1). On macOS a benign `unexpected errno: 19` / tcgetattr
# stack trace prints before the frame (the fallback path firing); exit is still 0
# and the dashboard renders — we tolerate it and assert the panels, not its absence.
set -uo pipefail
...
if ! ./build.sh cli >/tmp/dashboard-smoke-build.log 2>&1; then
  echo "RESULT: FAIL (build) — see /tmp/dashboard-smoke-build.log"; exit 1
fi
BIN=zig-out/bin/abi
...
if [ "$fail" -eq 0 ]; then
  ...
  echo "  (a benign 'unexpected errno: 19' trace before the frame is normal on macOS; exit stays 0)"
  exit 0
fi
echo "RESULT: FAIL"
exit 1
