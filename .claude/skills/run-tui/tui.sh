...
# dashboard through a tmux pty. `abi dashboard` (and `abi tui` / `abi --tui`)
# need a real terminal — piped stdin makes tcgetattr fail (errno 19) and it
# stack-traces. tmux gives it a pty: we launch it, capture the rendered pane,
# assert the dashboard painted, send the quit key, and tear the session down.
#
