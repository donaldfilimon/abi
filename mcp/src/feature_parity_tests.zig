...
fn getMissingDecls(comptime Expected: type, comptime Actual: type) []const []const u8 {
    ...
    inline for (expected_decls) |decl| {
        ...
        inline for (actual_decls) |actual_decl| {
            if (comptime std.mem.eql(u8, decl.name, actual_decl.name)) {
                found = true;
                // Deep parity check
                const exp_decl = @field(Expected, decl.name);
                const act_decl = @field(Actual, actual_decl.name);
                if (@TypeOf(exp_decl) != @TypeOf(act_decl)) {
                    // Mismatched types could be caught here
                }
            }
        }
        if (!found) {
...
/// Produces a compile error listing all missing declarations.
fn assertParity(comptime name: []const u8, comptime Mod: type, comptime Stub: type) void {
    const mod_missing = comptime getMissingDecls(Mod, Stub);
    const stub_missing = comptime getMissingDecls(Stub, Mod);

    if (mod_missing.len > 0 or stub_missing.len > 0) {
        comptime var msg: []const u8 = "\nParity Drift in " ++ name ++ ":";
        if (mod_missing.len > 0) {
            msg = msg ++ "\n  Missing in stub.zig:";
            inline for (mod_missing) |m| {
                msg = msg ++ "\n    - " ++ m;
            }
        }
        if (stub_missing.len > 0) {
            msg = msg ++ "\n  Missing in mod.zig:";
            inline for (stub_missing) |m| {
                msg = msg ++ "\n    - " ++ m;
            }
        }
        @compileError(msg);
