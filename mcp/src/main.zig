...
const llm_plugin_manifest = @import("features/ai/llm/providers/manifest.zig");
...
pub fn printVersion() void {
    printSharedCliText(cli.writeVersion) catch |err| {
        std.debug.print("Warning: Failed to write version: {}\n", .{err});
    };
}
...
pub fn printHelp() void {
    printSharedCliText(cli.writeHelp) catch |err| {
        std.debug.print("Warning: Failed to write help: {}\n", .{err});
    };
}
...
pub fn runDoctor(allocator: std.mem.Allocator) !void {
    ...
    _ = runShellDiagnostic(allocator,
        ...
        std.debug.print("  failed to summarize MCP config: {}\n", .{err});
    };
