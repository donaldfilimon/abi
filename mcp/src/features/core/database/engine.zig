...
pub const Engine = struct {
    ...
    pub fn searchByVector(
        ...
        // When HNSW graph has no entry point (insert shim mode), fall back to
        // brute-force linear scan so that vectors added via the compatibility
        // shim are still discoverable.
        if (matched_results.len == 0 and self.vectors_array.items.len > 0) {
            for (self.vectors_array.items) |item| {
            ...
            std.mem.sort(SearchResult, final_results.items, {}, struct {
                fn lessThan(_: void, a: SearchResult, b: SearchResult) bool {
                    return a.similarity > b.similarity;
                }
            }.lessThan);

            if (final_results.items.len > options.k) {
                final_results.shrinkRetainingCapacity(options.k);
            }
