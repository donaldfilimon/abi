...
test "Persistence round-trip save and load" {
    ...
    defer {
        var io_backend = std.Io.Threaded.init(allocator, .{ .environ = std.process.Environ.empty });
        const io = io_backend.io();
        std.Io.Dir.cwd().deleteFile(io, path) catch |cleanup_err| {
            std.log.warn("failed to remove persistence test file '{s}': {t}", .{ path, cleanup_err });
        };
        io_backend.deinit();
    }
...
test "Persistence invalid magic" {
    ...
    defer std.Io.Dir.cwd().deleteFile(io, path) catch |cleanup_err| {
        std.log.warn("failed to remove persistence test file '{s}': {t}", .{ path, cleanup_err });
    };
