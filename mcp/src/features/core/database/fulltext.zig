...
test "inverted index basic" {
    ...
    try index.indexDocument(2, "hello");
...
test "bm25 idf calculation" {
    ...
    // Add some documents.
    // Note: the Porter stemmer reduces "document" to "docu" and "another" to
    // "another", so we must query the stemmed forms when checking document
    // frequency directly.
    try index.indexDocument(1, "test docu");
    try index.indexDocument(2, "alpha docu");
    try index.indexDocument(3, "alpha beta");

    // "docu" appears in docs 1 and 2 (df=2), "test" appears only in doc 1 (df=1).
    const common_df = index.getDocumentFrequency("docu");
