...
pub fn run(allocator: std.mem.Allocator, config: AbbyTrainConfig) !void {
    ...
    if (config.wdbx_output_path) |wdbx_path| {
        ...
        var export_state = try training.ModelState.init(allocator, model.weights.numParams(), "abbey");
        defer export_state.deinit();

        if (brain_export.exportDual(allocator, &export_state, brain_cfg, meta)) |_| {
