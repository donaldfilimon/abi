...
const http_plugin = @import("http_plugin.zig");
...
fn generateHttpPlugin(allocator: std.mem.Allocator, cfg: types.GenerateConfig) !types.GenerateResult {
    const entry = try loader.findEnabledByKind(allocator, .http, cfg.plugin_id) orelse return errors.ProviderError.PluginNotFound;
    defer {
        var mutable = entry;
        mutable.deinit(allocator);
    }
    return http_plugin.generate(allocator, entry, cfg);
}

fn generateNativePlugin(allocator: std.mem.Allocator, cfg: types.GenerateConfig) !types.GenerateResult {
    const entry = try loader.findEnabledByKind(allocator, .native, cfg.plugin_id) orelse return errors.ProviderError.PluginNotFound;
    defer {
        var mutable = entry;
        mutable.deinit(allocator);
    }
    return native_plugin.generate(allocator, entry, cfg);
}
