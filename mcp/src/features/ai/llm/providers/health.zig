...
pub fn isAvailable(
    ...
    return switch (provider) {
        ...
        .plugin_http => blk: {
            if (plugin_id == null) break :blk loader.hasAnyEnabled(.http);
            const match = loader.findEnabledByKind(allocator, .http, plugin_id) catch null;
            if (match) |entry| {
                var owned = entry;
        ...
        .plugin_native => blk: {
            if (plugin_id == null) break :blk loader.hasAnyEnabled(.native);
            const match = loader.findEnabledByKind(allocator, .native, plugin_id) catch null;
            if (match) |entry| {
                var owned = entry;
                defer owned.deinit(allocator);
                break :blk true;
