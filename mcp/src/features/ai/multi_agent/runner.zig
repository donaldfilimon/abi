                ...
                while (attempts <= self.config.max_retries) : (attempts += 1) {
                    if (agent) |ag| {
                        ...
                        chain.addStep(.assessment, "Evaluating step output confidence", .{
                            ...
                            std.log.warn("failed to record confidence assessment step: {t}", .{err});
                        };

                        chain.finalize() catch |err| {
                            std.log.warn("failed to finalize confidence assessment chain: {t}", .{err});
                        };
                        ...
                        if (final_conf.score < 0.5) {
                            if (attempts < self.config.max_retries) {
                                chain.addStep(.research, "Confidence below threshold, initiating research", .{
                                    ...
                                    std.log.warn("failed to record fallback research step: {t}", .{err});
                                };
