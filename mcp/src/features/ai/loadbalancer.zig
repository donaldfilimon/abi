...
//!
//! Manages a pool of profile instances, distributing requests based on
...
/// Represents a single profile instance in the load balancer.
pub const ProfileNode = struct {
    ...
    pub fn isAvailable(self: *const ProfileNode, cfg: config.LoadBalancingConfig) bool {
...
/// Combined score for profile selection.
...
pub const FallbackConfig = struct {
    /// Enable fallback to alternative profiles when primary is unavailable.
    enabled: bool = true,
    /// Ordered list of fallback profiles (if empty, use all available).
    fallback_order: []const types.ProfileType = &.{},
...
/// Implementation of the profile load balancer.
pub const ProfileLoadBalancer = struct {
    allocator: std.mem.Allocator,
    config: config.LoadBalancingConfig,
    fallback_config: FallbackConfig,
    nodes: std.AutoHashMapUnmanaged(types.ProfileType, ProfileNode),
    /// Circuit breaker registry for per-profile circuit breakers.
    ...
    /// Initialize a new profile load balancer.
    ...
    /// Register a profile in the load balancer.
    pub fn registerProfile(self: *Self, profile_type: types.ProfileType, weight: f32) !void {
        self.mutex.lock();
        defer self.mutex.unlock();

        try self.nodes.put(self.allocator, profile_type, .{
            .profile_type = profile_type,
            .weight = weight,
        });

        // Register a circuit breaker for this profile
        const name = @tagName(profile_type);
        try self.circuit_breakers.register(name, self.circuit_breakers.default_config);
    }

    /// Select the best available profile instance based on routing scores and node health.
    pub fn selectWithScores(self: *Self, scores: []const ProfileScore) !types.ProfileType {
        self.mutex.lock();
        defer self.mutex.unlock();

        self.stats.total_requests += 1;

        var best_profile: ?types.ProfileType = null;
        var max_combined_score: f32 = -1.0;

        for (scores) |ps| {
            if (self.nodes.getPtr(ps.profile_type)) |node| {
                // Check circuit breaker state
                const cb_name = @tagName(ps.profile_type);
    ...
    /// Select a fallback profile (unlocked version).
    fn selectFallbackUnlocked(self: *Self, original_scores: []const ProfileScore) ?types.ProfileType {
        // If fallback order is specified, use it
        if (self.fallback_config.fallback_order.len > 0) {
            for (self.fallback_config.fallback_order) |profile_type| {
                if (self.nodes.getPtr(profile_type)) |node| {
                    // Skip if it was in the original scores (already rejected)
                    var in_original = false;
                    for (original_scores) |ps| {
                        if (ps.profile_type == profile_type) {
                            in_original = true;
                            break;
                        }
                    }
        ...
        while (it.next()) |entry| {
            ...
            if (node.status != .circuit_broken and node.status != .unhealthy) {
                ...
                var was_tried = false;
                for (original_scores) |ps| {
                    if (ps.profile_type == entry.key_ptr.*) {
                        was_tried = true;
                        break;
                    }
                }
    ...
    /// Record a successful interaction for a profile.
    pub fn recordSuccess(self: *Self, profile_type: types.ProfileType) void {
        self.recordSuccessWithLatency(profile_type, 0);
    }

    /// Record a successful interaction with latency measurement.
    pub fn recordSuccessWithLatency(self: *Self, profile_type: types.ProfileType, latency_ms: u64) void {
        self.mutex.lock();
        defer self.mutex.unlock();
        ...
        const cb_name = @tagName(profile_type);
        if (self.circuit_breakers.getBreaker(cb_name)) |breaker| {
            breaker.recordSuccess();
        }
    }

    /// Record a failed interaction for a profile.
    pub fn recordFailure(self: *Self, profile_type: types.ProfileType) void {
        self.mutex.lock();
        defer self.mutex.unlock();
    ...
    /// Update health score for a profile (called by health checker).
    pub fn updateHealthScore(self: *Self, profile_type: types.ProfileType, score: f32) void {
        self.mutex.lock();
        defer self.mutex.unlock();
    ...
    /// Get node statistics.
    pub fn getNodeStats(self: *Self, profile_type: types.ProfileType) ?ProfileNode {
        self.mutex.lock();
        defer self.mutex.unlock();
        return self.nodes.get(profile_type);
    }
...
test "load balancer initialization" {
    ...
    var lb = ProfileLoadBalancer.init(allocator, .{});
    defer lb.deinit();

    try lb.registerProfile(.abbey, 1.0);
    try lb.registerProfile(.aviva, 1.0);
