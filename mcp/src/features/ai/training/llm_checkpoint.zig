...
const binary = @import("../../../foundation/mod.zig").utils.binary;
...
test "llm checkpoint roundtrip" {
    ...
    defer {
        ...
        defer io_backend.deinit();
        const io = io_backend.io();
        std.Io.Dir.cwd().deleteFile(io, test_path) catch |cleanup_err| {
            std.log.warn("failed to remove LLM checkpoint test file '{s}': {t}", .{ test_path, cleanup_err });
        };
    }
