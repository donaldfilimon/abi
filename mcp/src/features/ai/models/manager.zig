...
test "remove model deletes file" {
    ...
    var file = std.Io.Dir.cwd().createFile(io, test_file, .{ .truncate = true }) catch |err| {
        std.log.warn("createFile failed: {t}", .{err});
        return err;
    };
    file.close(io);

    errdefer std.Io.Dir.cwd().deleteFile(io, test_file) catch |cleanup_err| {
        std.log.warn("failed to remove model manager test file '{s}': {t}", .{ test_file, cleanup_err });
    };

    var manager = try Manager.init(allocator, .{ .auto_scan = false });
    defer manager.deinit();

    // Register with relative path (removeModel handles both relative and absolute)
