...
pub fn renderDashboard(screen: *Screen) void {
    ...
    if (header_area.height >= 2) {
        const version_text = "v" ++ build_options.package_version ++ " | Zig " ++ build_options.zig_version ++ " | Multi-Persona AI + WDBX";
...
pub fn run(allocator: std.mem.Allocator) !void {
    ...
    defer {
        ...
        var writer = std.Io.Writer.fixed(&buf);
        ansi.showCursor(&writer) catch |err| {
            std.log.warn("failed to prepare cursor restore sequence: {t}", .{err});
        };
        flushToFd(stdout_fd, buf[0..writer.end]);
    }
