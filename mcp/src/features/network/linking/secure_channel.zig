...
pub const SecureChannel = struct {
    ...
    fn noiseXXHandshake(self: *SecureChannel) ChannelError!void {
        ...
        var session_hash = std.crypto.hash.blake2.Blake2b256.init(.{});
    ...
    fn wireguardHandshake(self: *SecureChannel) ChannelError!void {
        ...
        var ck_hash = std.crypto.hash.blake2.Blake2b256.init(.{});
        ...
        if (self.config.psk) |psk| {
            var psk_hash = std.crypto.hash.blake2.Blake2b256.init(.{});
    ...
    fn tlsHandshake(self: *SecureChannel) ChannelError!void {
        ...
        var th = std.crypto.hash.blake2.Blake2b256.init(.{});
        th.update(&client_random);
        th.update(&server_seed);
        ...
        // Session ID for resumption
        var sid_hash = std.crypto.hash.blake2.Blake2b256.init(.{});
        sid_hash.update(&client_random);
        sid_hash.update(&server_seed);
    ...
    fn deriveNewKeys(self: *SecureChannel) ChannelError!void {
        var hash = std.crypto.hash.blake2.Blake2b256.init(.{});
