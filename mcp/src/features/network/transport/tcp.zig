...
pub const TcpTransport = struct {
    ...
    pub fn start(self: *TcpTransport) !void {
        ...
        // Bind
        const rc = std.posix.system.bind(listener, &addr.any, addr.getOsSockLen());
        if (rc != 0) {
            return TransportError.BindFailed;
        }

        // Listen
        const rc_listen = std.posix.system.listen(listener, 128);
        if (rc_listen != 0) {
    ...
    fn acceptLoop(self: *TcpTransport) void {
        ...
        while (self.running.load(.acquire)) {
            ...
            if (client_fd < 0) {
                // Handle as error, similar to previous catch behavior
                break;
            }
    ...
    fn handleConnection(self: *TcpTransport, client_fd: std.posix.socket_t, addr: std.posix.sockaddr) void {
        ...
        while (self.running.load(.acquire)) {
            ...
            while (total_read < MessageHeader.SIZE) {
                const n = std.posix.system.recv(
                    client_fd,
                    header_buf[total_read..].ptr,
                    header_buf[total_read..].len,
                    0,
                );
    ...
    fn handleRequest(
        ...
        std.log.debug("Transport: Response prepared: type={d} id={d} len={d}", .{
            response_type,
            header.request_id,
            response.len,
        });
