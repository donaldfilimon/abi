...
pub const ResponseBuilder = struct {
    ...
    pub fn setCookie(self: *ResponseBuilder, name: []const u8, value: []const u8, options: CookieOptions) !*ResponseBuilder {
        ...
        try cookie.appendSlice(self.allocator, "=");
        try cookie.appendSlice(self.allocator, value);

        if (options.path) |path| {
            try cookie.appendSlice(self.allocator, "; Path=");
            try cookie.appendSlice(self.allocator, path);
        }
        if (options.domain) |domain| {
            try cookie.appendSlice(self.allocator, "; Domain=");
            try cookie.appendSlice(self.allocator, domain);
        }
        if (options.max_age) |max_age| {
            try cookie.appendSlice(self.allocator, "; Max-Age=");
            try cookie.print(self.allocator, "{d}", .{max_age});
        }
        if (options.secure) {
        ...
        if (options.same_site) |same_site| {
            try cookie.appendSlice(self.allocator, "; SameSite=");
            try cookie.appendSlice(self.allocator, same_site);
        }
    ...
    pub fn jsonError(self: *ResponseBuilder, status: types.Status, message: []const u8) !*ResponseBuilder {
        ...
        self.body.clearRetainingCapacity();
        try self.body.print(self.allocator,
    ...
    pub fn build(self: *ResponseBuilder) ![]const u8 {
        ...
        try result.print(self.allocator, "HTTP/1.1 {d} ", .{@intFromEnum(self.status)});
        if (self.status.phrase()) |phrase| {
            try result.print(self.allocator, "{s}", .{phrase});
        } else {
            try result.appendSlice(self.allocator, "Unknown");
        }
        try result.appendSlice(self.allocator, "\r\n");

        if (self.body.items.len > 0) {
            try result.print(self.allocator, "Content-Length: {d}\r\n", .{self.body.items.len});
        }

        var it = self.headers.iterator();
        while (it.next()) |entry| {
            try result.print(self.allocator, "{s}: {s}\r\n", .{ entry.key_ptr.*, entry.value_ptr.* });
        }
