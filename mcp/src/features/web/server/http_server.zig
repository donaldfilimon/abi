...
pub const Server = struct {
    ...
    pub fn start(self: *Server) !void {
        ...
        // Create socket
        const sock = try std.posix.system.socket(
            std.posix.AF.INET,
            std.posix.SOCK.STREAM,
            0,
        );
        errdefer std.posix.system.close(sock);
        ...
        std.posix.system.bind(sock, sock_addr, @sizeOf(std.c.sockaddr.in)) catch
            return ServerError.BindFailed;

        // Listen
        std.posix.system.listen(sock, 128) catch return ServerError.BindFailed;
    ...
    pub fn accept(self: *Server) !Connection {
        ...
        const client_sock = std.posix.system.accept(sock, &client_addr, &addr_len, 0) catch |err| {
        ...
        // std.posix.system.close(client_sock);
    ...
    fn handleConnection(self: *Server, conn: *Connection) void {
        defer {
            std.posix.system.close(conn.sock);
            self.mutex.lock();
            self.stats.active_connections -= 1;
            self.mutex.unlock();
        }
        ...
        const bytes_read = std.posix.system.read(conn.sock, &buffer) catch |err| {
        ...
        const bytes_written = std.posix.system.write(conn.sock, out_buffer.items) catch |err| {
