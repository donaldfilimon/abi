...
const health = if (build_options.feat_ai and @hasDecl(ai_mod, "health")) ai_mod.health else struct {
...
fn handleHealthCheck(ctx: *RouteContext) RouteError!void {
    var response_obj = try std.json.ObjectMap.init(ctx.allocator, &.{}, &.{});
    defer response_obj.deinit(ctx.allocator);

    try response_obj.put(ctx.allocator, "status", std.json.Value{ .string = "ok" });
    try response_obj.put(ctx.allocator, "timestamp", std.json.Value{ .integer = time.unixSeconds() });

    // Add health details if checker is available
    if (ctx.health_checker) |checker| {
        const has_health = build_options.feat_ai and @hasDecl(ai_mod, "health");
        if (has_health) {
            const aggregate = checker.getAggregateHealth();
            try response_obj.put(ctx.allocator, "healthy_profiles", std.json.Value{ .integer = @intCast(aggregate.healthy_count) });
            try response_obj.put(ctx.allocator, "degraded_profiles", std.json.Value{ .integer = @intCast(aggregate.degraded_count) });
            try response_obj.put(ctx.allocator, "unhealthy_profiles", std.json.Value{ .integer = @intCast(aggregate.unhealthy_count) });
            ...
            try response_obj.put(ctx.allocator, "overall_status", std.json.Value{ .string = status_str });
        }
    }
