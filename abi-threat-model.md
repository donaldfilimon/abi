...
- **Credentials store** — `src/foundation/credentials.zig`. Plaintext JSON at `~/.abi/credentials.json`; managed by `abi auth`.
...
| TM-013 | Reachable network peer | `wdbx cluster serve` bound to a routable host (`0.0.0.0`/NIC) | Forge `RequestVote`/`AppendEntries` on the consensus port | Inject votes / append arbitrary log entries → corrupt replicated state | WDBX consensus state | **No authentication** on the consensus transport (`cluster_rpc.zig`); loopback default; non-loopback bind now prints an exposure warning (`wdbx_runtime.zig`) | No shared secret/token/mTLS on the RPC; any reachable peer is trusted | Keep loopback unless on a trusted segment; add a shared cluster secret or mTLS before any routable bind; restrict via network ACL | Warn on non-loopback bind (done); log peer addrs | Medium | High | **high** |
