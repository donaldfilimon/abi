...
- **Remote/off-box client → MCP HTTP `/message`** — data: JSON-RPC tool calls (prompts, file paths, plugin names). Channel: HTTP (plaintext) on loopback, fronted/exposed per operator. Guarantees: 64 KB size cap (`protocol.MAX_REQUEST_SIZE`), leading-`{` structural pre-check, shared JSON nesting depth bound (`protocol.MAX_JSON_DEPTH`, currently 32) before parse, and optional bearer-token enforcement via `ABI_MCP_HTTP_TOKEN`; no TLS, per-route authorization, or origin check. Validation: `std.json.parseFromSlice` with `ignore_unknown_fields=true` after structural checks.
...
- **MCP tool `ai_train` → filesystem** — data: dataset read path, artifact write dir. Channel: file I/O. Guarantees: non-empty + profile validation; middleware rejects `..` segments; training path confines dataset/artifact under cwd or `ABI_TRAIN_DATA_ROOT` with realpath/symlink-escape rejection (`training_support.confineTrainingPath`). Paths outside the root are rejected; residual: missing-leaf lexical check has TOCTOU character before open; `evaluate` config-only path is unconfined.
- **MCP tool `connector_test` → connectors** — uses `.local` transport + dummy keys (`connector_tools.zig`); no real egress.
- **CLI `auth signin` → credentials file** — data: secrets from stdin. Channel: file write `~/.abi/credentials.json`; POSIX-capable targets create/repair `~/.abi` as `0o700` and open/truncate the credentials file as `0o600` before writing secrets; POSIX TTYs disable terminal echo during secret entry and restore it afterward (Windows: no echo-suppress path — disclosed).
- **Connector `.live` → external API** — data: API keys (Authorization/x-api-key/Basic), prompts, message content. Channel: `std.http.Client.fetch`; `joinUrl` requires `https://` base scheme (`requireHttpsBaseUrl`); non-HTTPS returns `InsecureBaseUrl`. No host allowlist / pinning.
- **Disk (JSONL/WAL/snapshot) → WDBX load** — on-disk data parsed on startup; WAL frames CRC32-checked (`wal.zig`), snapshots SHA-256-checked (`persistence.zig`), vector arrays length-bounded; ambient durable-store parent dirs created/repaired as owner-only (`0700`) on POSIX (`durable_store.ensureOwnerOnlyDir`).
- **OS-control execute → host** — data: argv (no shell). Guarantees: allowlist + denylist + workspace containment + `--confirm` (`os_control/mod.zig`).
...
5. **Plaintext-HTTP key leak via misconfig.** A connector `base_url` set to `http://…` → `joinUrl` / `requireHttpsBaseUrl` now **reject** non-`https` schemes (`InsecureBaseUrl`). Residual: host allowlist / pinning still absent; operators can still point HTTPS at an attacker host.
...
9. **Shoulder/terminal capture of secret entry.** `abi auth signin` disables terminal echo on POSIX TTYs during secret entry; Windows still lacks an echo-suppress path → residual shoulder-surf risk on Windows consoles.
...
| TM-003 | Remote MCP caller | TM-001 reachable; tool inputs attacker-set | `ai_train` with chosen `dataset`/`artifact_dir` | Bounded file read/write as abi process | Host files, WDBX | Non-empty + profile validation; `..` rejected in middleware; train path confines under cwd/`ABI_TRAIN_DATA_ROOT` with realpath/symlink-escape rejection (`training_support.confineTrainingPath`) | Residual TOCTOU on missing leaves; `evaluate` unconfined; default root is cwd not a fixed sandbox | Optional fixed sandbox root in production configs; audit-log resolved paths | Audit-log resolved paths per train call | Medium | Medium | **medium** |
| TM-004 | Local user (co-resident) | Read access to home / weak OS permissions | Read `~/.abi/credentials.json` | Provider key theft → account abuse/impersonation | API keys | POSIX-capable targets create/repair `~/.abi` as `0o700` and write `credentials.json` as `0o600` before secret bytes (`credentials.zig`) | Plaintext JSON remains; no Windows ACL/keychain path; no memory zeroing | OS keychain/secret-service; Windows ACL hardening; zero buffers after use | FIM on credentials file; alert on non-owner reads | Low | High | **medium** |
...
| TM-006 | Local attacker w/ store write | Shared/writable store dir | Tamper a WAL frame + recompute CRC32 | Undetected integrity compromise of replayed state | WDBX integrity | CRC32 framing (`wal.zig`); SHA-256 snapshots; ambient durable-store parent dirs `0700` on POSIX (`durable_store`) | CRC32 is non-cryptographic (forgeable); leaf WAL files not forced `0600` | HMAC/keyed-MAC WAL frames; tighter leaf-file modes | Periodic snapshot-vs-WAL reconciliation alerts | Low | Medium | **medium** |
| TM-007 | Network eavesdropper | `base_url` set to `http://` (misconfig/untrusted config) | Capture API key sent over cleartext | Key disclosure | API keys | Strict cred-format validation; `.live` gating; `requireHttpsBaseUrl` / `joinUrl` reject non-`https` (`http.zig`) | No host allowlist / pinning | Per-provider host allowlist; pin where feasible | Egress monitoring for plaintext API hosts | Low | Medium | **low** |
| TM-008 | Remote/local caller | TM-001/TM-002 reachable | Send 64 KB deeply-nested JSON | CPU/stack pressure (DoS) | Availability | 64 KB request cap; leading-`{` precheck; `MAX_JSON_DEPTH` nesting bound on stdio + HTTP parse paths (`protocol.zig` / `rpc.zig`) | Bound is structural (brace walk), not full schema validation | Keep bound; optionally lower for stricter fronted deployments | Latency/CPU spikes per request size | Low | Low | **low** |
| TM-009 | Build/supply-chain actor | Write access to a plugin dir | `entry_point` as `.zig` symlink outside plugin dir | Out-of-tree code compiled into binary | Plugin integrity | `isSafeEntryPoint` blocks `..`/abs/`:` (`generate_plugin_registry.zig`) | Symlink target not resolved/contained | Resolve realpath and assert containment under plugin dir; reject symlinks | Build-time check / manifest review | Low | Medium | **low** |
| TM-010 | Shoulder-surfer / terminal capture | Access to operator terminal/scrollback | Observe secret typed at `auth signin` | Secret disclosure | API keys | POSIX TTY: termios echo disabled around secret read, restored after (`auth.zig`); Windows: no echo-suppress path (disclosed) | Non-TTY pipes still echo nothing to a terminal but secrets may appear in process/script capture; Windows residual | Windows console mode / secure input APIs | n/a | Low | Low | **low** |
...
| `src/cli/handlers/auth.zig` | Secret entry; POSIX no-echo; Windows gap | TM-010 |
| `src/connectors/http.zig` | Live URL join; HTTPS scheme required | TM-007 |
| `src/features/ai/training_support.zig` | Train path confinement / `ABI_TRAIN_DATA_ROOT` | TM-003 |
| `src/features/wdbx/durable_store.zig` | Parent-dir `0700` on open | TM-006 |
| `src/mcp/protocol.zig` | Request size + JSON depth bounds | TM-008 |
