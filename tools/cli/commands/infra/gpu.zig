...
fn printDevices(allocator: std.mem.Allocator) !void {
    if (!gpu_detect.moduleEnabled()) {
        utils.output.printWarning("GPU Devices: disabled (build without -Dfeat-gpu=true)", .{});
        return;
    }
...
fn printDefaultDevice(allocator: std.mem.Allocator) !void {
    if (!gpu_detect.moduleEnabled()) {
        utils.output.printWarning("GPU default device: disabled (build without -Dfeat-gpu=true)", .{});
        return;
    }
...
fn printStatus(allocator: std.mem.Allocator) !void {
    if (!gpu_detect.moduleEnabled()) {
        utils.output.printWarning("GPU status: disabled (build without -Dfeat-gpu=true)", .{});
        return;
    }
