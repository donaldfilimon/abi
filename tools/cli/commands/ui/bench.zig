...
const DashboardState = struct {
    ...
    pub fn init(
        ...
        return .{
            ...
            .panel = tui.BenchmarkPanel.init(allocator, terminal, theme_manager.current),
...
fn renderDashboard(state: *DashboardState) !void {
    ...
    try term.write(" ABI BENCHMARK DASHBOARD ");
    try term.write(theme_val.reset);
    ...
    try term.write(" [q]uit  [p]ause  [up/down]select  [t]heme  [?]help");
    try term.write(theme_val.reset);
}
