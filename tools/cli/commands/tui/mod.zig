...
fn computeVisibleRows(rows: u16) usize {
    return tui_layout.computeVisibleRows(rows);
}

fn clampedFrameWidth(cols: u16) usize {
    return tui_layout.clampedFrameWidth(cols);
}

fn completionDropdownRowCount(state: *const TuiState) u16 {
    return tui_layout.completionDropdownRowCount(state);
}

fn menuStartRow(state: *const TuiState) u16 {
    return tui_layout.menuStartRow(state);
}
...
fn clickedIndexFromRow(
    ...
    return tui_layout.clickedIndexFromRow(
        row0,
        menu_start_row,
        show_top_indicator,
        scroll_offset,
