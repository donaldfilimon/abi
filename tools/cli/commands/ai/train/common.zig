...
pub fn downloadToFile(allocator: std.mem.Allocator, url: []const u8, path: []const u8, max_bytes: usize) !void {
    var client = try abi.foundation.utils.async_http.AsyncHttpClient.init(allocator);
    defer client.deinit();

    var request = try abi.foundation.utils.async_http.HttpRequest.init(allocator, .get, url);
