...
//!
//! Reads state from `.ralph/state.zon` and `.ralph/skills.zonl`
...
pub const RalphPanel = struct {
    ...
    fn loadState(self: *RalphPanel) void {
        ...
        // Reads .ralph/state.zon for run count, gate status, and lock state.
        self.loadStateJson();
        self.is_locked = fileExists(".ralph/lock");
        self.skill_count = countLines(".ralph/skills.zonl");
    }

    fn loadStateJson(self: *RalphPanel) void {
        const data = readSmallFile(".ralph/state.zon", self.allocator, 4096) orelse return;
        defer self.allocator.free(data);
        if (std.zon.parseFromSlice(StateJson, self.allocator, data, .{ .ignore_unknown_fields = true })) |parsed| {
            defer parsed.deinit();
