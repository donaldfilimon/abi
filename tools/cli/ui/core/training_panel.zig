...
pub const TrainingPanel = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, theme: *const themes.Theme, config: PanelConfig) TrainingPanel {
        const path = std.fmt.allocPrint(allocator, "{s}/metrics.zonl", .{config.log_dir}) catch config.log_dir;
    ...
    fn loadRunAtIndex(self: *TrainingPanel, index: usize) void {
        ...
        const run_path = std.fmt.allocPrint(self.allocator, "{s}/{s}/metrics.zonl", .{
            self.config.log_dir,
            run_name,
        }) catch return;
    ...
    fn scanAvailableRuns(self: *TrainingPanel) !void {
        ...
        while (true) {
            ...
            if (entry.kind == .directory) {
                // Check if directory has a metrics.zonl file
                const metrics_path = std.fmt.allocPrint(self.allocator, "{s}/{s}/metrics.zonl", .{
                    self.config.log_dir,
                    entry.name,
                }) catch continue;
