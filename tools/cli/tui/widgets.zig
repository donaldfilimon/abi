...
pub const ProgressIndicator = struct {
    ...
    pub fn init(term: *terminal.Terminal, message: []const u8, style: SpinnerStyle) ProgressIndicator {
        return .{
            ...
            .frame = 0,
            .active = false,
            .start_time = abi.services.shared.utils.unixMs(),
        };
    }

    pub fn start(self: *ProgressIndicator) !void {
        self.active = true;
        self.start_time = abi.services.shared.utils.unixMs();
        try self.render();
    }
    ...
    fn render(self: *ProgressIndicator) !void {
        ...
        // Show elapsed time
        const elapsed = abi.services.shared.utils.unixMs() - self.start_time;
