...
test "parseMetaBlock handles forward command without aliases or subcommands" {
    const source =
        \\pub const meta: command_mod.Meta = .{
        \\    .name = "shell",
        \\    .description = "Open the canonical UI shell",
        \\    .io_mode = .io,
        \\    .forward = .{
        \\        .target = "ui",
        \\        .prepend_args = &[_][:0]const u8{},
        \\        .warning = "'abi shell' is deprecated; use 'abi ui'.",
        \\    },
        \\};
    ;
    ...
    try std.testing.expectEqualStrings("shell", cmd.name);
    try std.testing.expectEqualStrings("Open the canonical UI shell", cmd.description);
