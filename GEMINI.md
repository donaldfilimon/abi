...
- **GPU Backend Selection**: Vulkan devices are scored by type (discrete > integrated > virtual > cpu > other) during initialization. The `stdgpu` backend provides a CPU-based software fallback that's always available. See `src/compute/gpu/backends/vulkan_init.zig:selectPhysicalDevice()` and `src/compute/gpu/backends/stdgpu.zig:getDeviceInfo()`.
