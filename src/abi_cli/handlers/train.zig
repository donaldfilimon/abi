...
pub fn handleComplete(allocator: std.mem.Allocator, input: []const u8) !u8 {
    ...
    std.debug.print("model={s} profile={s} audit_passed={s}\n", .{ result.model, result.selected_profile.label(), if (result.audit.passed) "true" else "false" });
    if (result.query_vector_id) |qid| std.debug.print("query_vector_id={d}\n", .{qid});
