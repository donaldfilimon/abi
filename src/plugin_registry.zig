...
pub fn registerPlugins(registry: *Registry) !void {
    ...
    try registry.registerPlugin(.{ .name = "ai-plugin", .version = "0.1.0", .description = "Example reference plugin targeting the feat-ai gate.", .target_feature = "ai", .entry_point = "mod.zig", .commands = &.{
        .{ .name = "profile", .summary = "Show the current AI profile and router status", .aliases = &.{"ai-status"} },
        .{ .name = "models", .summary = "List available model backends" },
    } });
    try registry.registerPlugin(.{ .name = "example-plugin", .version = "0.1.0", .description = "Minimal example plugin used by registry generation tests.", .target_feature = "plugins", .entry_point = "mod.zig", .commands = &.{
        .{ .name = "echo", .summary = "Echo back the input text", .aliases = &.{"say"} },
    }, .context_providers = &.{
        .{ .name = "example-info", .summary = "Example plugin metadata for REPL context" },
    } });
    ...
    try registry.registerPlugin(.{ .name = "gpu-plugin", .version = "0.1.0", .description = "Example reference plugin targeting the feat-gpu gate.", .target_feature = "gpu", .entry_point = "mod.zig", .commands = &.{
        .{ .name = "gpu-status", .summary = "Report GPU backend and acceleration mode", .aliases = &.{"gpu"} },
        .{ .name = "gpu-info", .summary = "Show detailed GPU capabilities" },
    } });
    ...
    try registry.registerPlugin(.{ .name = "nn-plugin", .version = "0.1.0", .description = "Example reference plugin targeting the feat-nn gate.", .target_feature = "nn", .entry_point = "mod.zig", .commands = &.{
        .{ .name = "nn-train", .summary = "Train the miniature character-level neural-net demo" },
        .{ .name = "nn-sample", .summary = "Generate text from the trained neural-net model" },
    } });
    ...
    try registry.registerPlugin(.{ .name = "tui-plugin", .version = "0.1.0", .description = "Example reference plugin targeting the feat-tui gate.", .target_feature = "tui", .entry_point = "mod.zig", .commands = &.{
        .{ .name = "hello", .summary = "Greet from the tui-plugin", .aliases = &.{"hi"} },
        .{ .name = "ping", .summary = "Respond with pong and timing info" },
    } });
}
