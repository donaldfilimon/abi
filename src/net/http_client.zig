...
pub const HttpClient = struct {
    ...
    pub fn request(self: *Self, method: []const u8, url: []const u8, content_type: ?[]const u8, body: ?[]const u8) !HttpResponse {
        ...
        while (attempt <= self.config.max_retries) {
            if (self.config.verbose) {
                core.log.debug("HTTP {s} attempt {d}/{d}: {s}", .{ method, attempt + 1, self.config.max_retries + 1, url });
            }
            ...
            switch (result) {
                ...
                .timeout, .connection_failed, .network_error => {
                    if (attempt >= self.config.max_retries) {
                        if (self.config.verbose) {
                            core.log.err("HTTP request failed after {d} attempts", .{attempt + 1});
                        }
                        return error.HttpRequestFailed;
                    }

                    if (self.config.verbose) {
                        core.log.debug("HTTP request failed, retrying in {d}ms...", .{backoff_ms});
                    }
    ...
    pub fn testConnectivity(self: *Self, url: []const u8) !bool {
        if (self.config.verbose) {
            core.log.debug("Testing connectivity to: {s}", .{url});
        }

        var response = self.get(url) catch |err| {
            if (self.config.verbose) {
                core.log.warn("Connectivity test failed: {any}", .{err});
            }
            return false;
        };
        ...
        if (self.config.verbose) {
            core.log.debug("Connectivity test successful - status: {d}, body length: {d}", .{ response.status_code, response.body.len });
        }
...
pub const ConnectivityTester = struct {
    ...
    pub fn runDiagnostics(self: *Self, base_url: []const u8) !void {
        core.log.info("=== HTTP Client Connectivity Diagnostics ===", .{});

        // Test basic connectivity
        core.log.info("1. Testing basic connectivity...", .{});
        ...
        if (self.client.testConnectivity(health_url)) |success| {
            if (success) {
                core.log.info("   ✅ Basic connectivity successful", .{});
            } else {
                core.log.warn("   ❌ Basic connectivity failed", .{});
            }
        } else |err| {
            core.log.err("   ❌ Connectivity test error: {any}", .{err});
        }

        // Test with retries
        core.log.info("2. Testing with retry mechanism...", .{});
        const response = self.client.get(health_url) catch |err| {
            core.log.err("   ❌ GET request failed: {any}", .{err});
            return;
        };
        defer response.deinit();

        core.log.info("   ✅ GET request successful - Status: {d}", .{response.status_code});
        core.log.info("   📊 Response headers: {d}", .{response.headers.count()});
        core.log.info("   📄 Response body length: {d} bytes", .{response.body.len});

        // Test POST request
        core.log.info("3. Testing POST request...", .{});
        const test_data = "{\"test\": \"connectivity\"}";
        const post_response = self.client.post(health_url, "application/json", test_data) catch |err| {
            core.log.err("   ❌ POST request failed: {any}", .{err});
            return;
        };
        defer post_response.deinit();

        core.log.info("   ✅ POST request successful - Status: {d}", .{post_response.status_code});

        core.log.info("=== Diagnostics Complete ===", .{});
    }
};
