...
pub const Config = struct {
    ...
    fn hasAi(self: Config) bool {
        return self.ai != null;
    }

    fn hasAiSubconfig(self: Config, comptime field_name: []const u8) bool {
        return if (self.ai) |ai| @field(ai, field_name) != null else false;
    }
    ...
    pub fn isEnabled(self: Config, feature: Feature) bool {
        return switch (feature) {
            ...
            .self_improve,
            => self.hasAi(),
            .llm => self.hasAiSubconfig("llm"),
            .embeddings => self.hasAiSubconfig("embeddings"),
            .agents => self.hasAiSubconfig("agents"),
            .training => self.hasAiSubconfig("training"),
            .explore => self.hasAi() and build_options.feat_explore,
            .abbey,
            .reasoning,
            .constitution,
            .pipeline,
            .eval,
            .rag,
            => self.hasAiSubconfig("reasoning") and build_options.feat_reasoning,
            ...
            .profiles,
            .profile,
            => self.hasAiSubconfig("profiles"),
            .vision => self.hasAi() and build_options.feat_vision,
...
pub const Builder = struct {
    ...
    pub fn with(self: *Builder, comptime feature: Feature, cfg: anytype) *Builder {
        ...
        switch (feature) {
            ...
            => {
                if (self.config.ai == null) self.config.ai = .{};
                self.config.ai.?.profiles = typed_cfg;
            },
            .abbey,
            .reasoning,
            .constitution,
            ...
            => {
                if (self.config.ai == null) self.config.ai = .{};
                self.config.ai.?.reasoning = typed_cfg;
            },
            .ai => self.config.ai = typed_cfg,
            .core,
            .streaming,
            .explore,
            .tools,
    ...
    pub fn withDefault(self: *Builder, comptime feature: Feature) *Builder {
        ...
        switch (feature) {
            .ai => self.config.ai = AiConfig.defaults(),
            .llm => {
                if (self.config.ai == null) self.config.ai = .{};
            ...
            .embeddings => {
                if (self.config.ai == null) self.config.ai = .{};
                self.config.ai.?.embeddings = EmbeddingsConfig.defaults();
            },
            .agents => {
                if (self.config.ai == null) self.config.ai = .{};
                self.config.ai.?.agents = AgentsConfig.defaults();
            },
            .training => {
                if (self.config.ai == null) self.config.ai = .{};
                self.config.ai.?.training = TrainingConfig.defaults();
            },
            .profiles,
            .profile,
            => {
                if (self.config.ai == null) self.config.ai = .{};
                self.config.ai.?.profiles = .{};
            },
            .abbey,
            .reasoning,
            .constitution,
            ...
            => {
                if (self.config.ai == null) self.config.ai = .{};
                self.config.ai.?.reasoning = AiConfig.ReasoningConfig.defaults();
            },
            .core,
            .streaming,
            .explore,
