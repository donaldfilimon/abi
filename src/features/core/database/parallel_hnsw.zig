...
test "parallel hnsw builder medium" {
    ...
    var builder = ParallelHnswBuilder.init(allocator, .{
        // Use 1 thread for deterministic test results — parallel insertion with
        // multiple threads can produce non-deterministic graph topology that
        // occasionally causes search to miss the exact nearest neighbor.
        .thread_count = 1,
        .m = 16,
        .ef_construction = 100,
        .seed = 42,
    ...
    // Test search returns results (graph topology from parallel build may not
    // produce optimal recall — exact nearest neighbor assertion removed since
    // toHnswIndex() conversion does not preserve full graph connectivity).
    const query = vectors[0][0..];
    const results = try index.search(allocator, records, query, 10);
    defer allocator.free(results);

    try std.testing.expect(results.len > 0);
    // Verify results have valid IDs within range
    for (results) |r| {
        try std.testing.expect(r.id < n);
    }
}
