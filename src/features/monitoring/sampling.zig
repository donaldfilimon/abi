...
extern "kernel32" fn QueryPerformanceCounter(lpPerformanceCount: *i64) callconv(.Stdcall) i32;
extern "kernel32" fn QueryPerformanceFrequency(lpFrequency: *i64) callconv(.Stdcall) i32;
extern "kernel32" fn GetCurrentProcess() callconv(.Stdcall) *anyopaque;
extern "kernel32" fn GetProcessTimes(hProcess: *anyopaque, lpCreationTime: *u64, lpExitTime: *u64, lpKernelTime: *u64, lpUserTime: *u64) callconv(.Stdcall) i32;
extern "kernel32" fn GlobalMemoryStatusEx(lpBuffer: *MEMORYSTATUSEX) callconv(.Stdcall) i32;
extern "psapi" fn GetProcessMemoryInfo(hProcess: *anyopaque, ppsmemCounters: *PROCESS_MEMORY_COUNTERS, cb: u32) callconv(.Stdcall) i32;
...
const SystemInfo = struct {
    ...
    fn getCpuUsageWindows() f64 {
        // Windows CPU usage using performance counters
        var creation_time: u64 = 0;
        var exit_time: u64 = 0;
        var kernel_time: u64 = 0;
        var user_time: u64 = 0;
        ...
        const total_cpu_time = (kernel_time + user_time) / 10_000_000; // Convert from 100ns to seconds
        ...
        // This is a simplified calculation - in practice, you'd need to track previous measurements
        // For now, return a basic estimate
        if (total_cpu_time > 0) {
            return @min(100.0, @as(f64, @floatFromInt(total_cpu_time)) * 10.0); // Rough estimate
        }
        return 0.0;
    }
    ...
    fn getMemoryInfoWindows() !struct { total: u64, used: u64, available: u64 } {
        ...
        if (GlobalMemoryStatusEx(&mem_status) == 0) {
            return error.MemoryQueryFailed;
        }

        const total = mem_status.ullTotalPhys;
        const available = mem_status.ullAvailPhys;
    ...
    fn getProcessInfoWindows() !struct { cpu_percent: f64, memory_rss: u64, memory_vms: u64, threads: u32, uptime: u64 } {
        ...
        // Get memory info
        var mem_counters = PROCESS_MEMORY_COUNTERS{
        ...
        if (GetProcessMemoryInfo(process, &mem_counters, @sizeOf(PROCESS_MEMORY_COUNTERS)) == 0) {
            return error.ProcessMemoryQueryFailed;
        }

        // Get CPU usage (simplified)
        const cpu_percent = getCpuUsageWindows();

        // Get creation time for uptime calculation
        var creation_time: u64 = 0;
        var exit_time: u64 = 0;
        var kernel_time: u64 = 0;
        var user_time: u64 = 0;
        ...
        if (GetProcessTimes(process, &creation_time, &exit_time, &kernel_time, &user_time) == 0) {
            return error.ProcessTimeQueryFailed;
        }

        const current_time = std.time.timestamp();
        const uptime = if (creation_time > 0) @as(u64, @intCast(current_time)) - (creation_time / 10_000_000) else 0;

        return .{
            .cpu_percent = cpu_percent,
            .memory_rss = mem_counters.WorkingSetSize,
            .memory_vms = mem_counters.PagefileUsage,
            .threads = 0, // Would need additional API calls
            .uptime = uptime,
        };
    }

    fn getDiskInfoWindows() !struct { read_bytes: u64, write_bytes: u64, usage_percent: f64 } {
        // Windows disk I/O monitoring requires performance counters or WMI
        // For now, return placeholder values
        // Real implementation would use:
        // - QueryPerformanceCounter with disk performance objects
        // - GetDiskFreeSpaceEx for usage percentage
