...
pub const ComputePipeline = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, compute_shader: Shader) Self {
        return .{
            ...
            .bind_groups = std.ArrayList(BindGroup).initCapacity(allocator, 0) catch unreachable,
            .allocator = allocator,
        };
    }
...
pub const BindGroup = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) Self {
        return .{
            .handle = GPUHandle{ .id = 1, .generation = 1 },
            .buffers = std.ArrayList(Buffer).initCapacity(allocator, 0) catch unreachable,
            .allocator = allocator,
        };
    }
...
pub const GPURenderer = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, config: GPUConfig) !*Self {
        ...
        self.* = .{
            ...
            .buffers = std.ArrayList(Buffer).initCapacity(allocator, 0) catch unreachable,
            .shaders = std.ArrayList(Shader).initCapacity(allocator, 0) catch unreachable,
            .compute_pipelines = std.ArrayList(ComputePipeline).initCapacity(allocator, 0) catch unreachable,
    ...
    pub fn computeMatrixMultiply(self: *Self, a: []const f32, b: []const f32, result: []f32, m: u32, n: u32, k: u32) !void {
        ...
        // For now, fall back to simple CPU implementation
        // Note: In production, this should use SIMD acceleration
        for (0..m) |i| {
            for (0..k) |j| {
                var sum: f32 = 0.0;
                for (0..n) |l| {
