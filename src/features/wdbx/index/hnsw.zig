...
pub const HnswNode = struct {
    id: u32,
    level: usize,
    edges: [MAX_LAYERS]std.ArrayListUnmanaged(u32) = initEdges(),
    lock: sync.SpinLock = .{},

    fn initEdges() [MAX_LAYERS]std.ArrayListUnmanaged(u32) {
...
pub const VectorStorage = struct {
    ...
    pub fn insert(self: *VectorStorage, id: u32, values: []const f32) !void {
        ...
        if (needed > self.data.items.len) {
            ...
            @memset(self.data.items[self.data.items.len..new_cap], 0);
        }
    ...
    pub fn contains(self: *const VectorStorage, id: u32) bool {
        return (id + 1) * self.dimensions <= self.data.items.len;
    }
};
...
pub fn cosineDistanceSIMD(a: []const f32, b: []const f32) f32 {
    ...
    while (i < simd_len) : (i += simd_width) {
        const va: @Vector(simd_width, f32) = a[i .. i + simd_width].*;
        const vb: @Vector(simd_width, f32) = b[i .. i + simd_width].*;
...
pub fn HnswIndex(comptime D: usize) type {
    return struct {
        ...
        nodes: std.ArrayListUnmanaged(HnswNode) = .empty,
        entry_node: ?u32 = null,
        lock: sync.RwLock = .{},
        max_level: usize = 0,
        ...
        pub fn insert(self: *Self, id: u32, values: []const f32) !void {
            ...
            var node = HnswNode{ .id = id, .level = level };
            var i: usize = 0;
            while (i <= level) : (i += 1) {
                try node.edges[i].initCapacity(self.allocator, M);
            }
        ...
        fn connectNodes(self: *Self, from: usize, to: usize, level: usize) !void {
            ...
            if (from_edges.items.len < M) {
                try from_edges.append(self.allocator, to);
            }
            ...
            if (to_edges.items.len < M) {
                try to_edges.append(self.allocator, from);
            }
        }

        pub fn search(self: *const Self, query: []const f32, limit: usize) ![]wdbx_mod.SearchResult {
            ...
            try visited.put(self.allocator, self.entry_node.?, {});
...
test "HnswNode deinit" {
    var node = HnswNode{ .id = 42, .level = 2 };
