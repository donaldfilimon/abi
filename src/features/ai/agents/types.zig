//! Type definitions for Agents stub module.

const std = @import("std");
const config_module = @import("../../../core/config/mod.zig");
const llm_providers = @import("../llm/types.zig").providers;
...
pub const ErrorContext = struct {
    ...
    pub fn apiError(err: AgentError, backend: AgentBackend, endpoint_val: []const u8, status: ?u16, model_val: ?[]const u8) ErrorContext {
        return .{ .@"error" = err, .backend = backend, .operation = .api_request, .http_status = status, .endpoint = endpoint_val, .model = model_val };
    }
    pub fn configError(err: AgentError, message_val: []const u8) ErrorContext {
        return .{ .@"error" = err, .backend = .echo, .operation = .configuration_validation, .message = message_val };
    }
    pub fn generationError(err: AgentError, backend: AgentBackend, model_val: []const u8, message_val: ?[]const u8) ErrorContext {
        return .{ .@"error" = err, .backend = backend, .operation = .response_generation, .model = model_val, .message = message_val };
    }
    pub fn retryError(err: AgentError, backend: AgentBackend, endpoint_val: []const u8, attempt: u32, max_attempts: u32) ErrorContext {
        return .{ .@"error" = err, .backend = backend, .operation = .api_request, .endpoint = endpoint_val, .retry_attempt = attempt, .max_retries = max_attempts };
    }
    pub fn format(_: ErrorContext, _: anytype) !void {}
    pub fn formatToString(_: ErrorContext, allocator: std.mem.Allocator) ![]u8 {
        return allocator.dupe(u8, "disabled");
    }
    pub fn log(_: ErrorContext) void {}
};
...
pub const AgentConfig = struct {
    ...
    pub fn validate(self: AgentConfig) AgentError!void {
        ...
        if (self.temperature < MIN_TEMPERATURE or self.temperature > MAX_TEMPERATURE) return AgentError.InvalidConfiguration;
        if (self.top_p < MIN_TOP_P or self.top_p > MAX_TOP_P) return AgentError.InvalidConfiguration;
        if (self.max_tokens == 0 or self.max_tokens > MAX_TOKENS_LIMIT) return AgentError.InvalidConfiguration;
    }
};
...
pub const ToolResult = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, success_val: bool, output_val: []const u8) ToolResult {
        return .{ .allocator = allocator, .success = success_val, .output = output_val, .error_message = null, .metadata = null };
    }
    pub fn fromError(allocator: std.mem.Allocator, err: []const u8) ToolResult {
        return .{ .allocator = allocator, .success = false, .output = "", .error_message = err, .metadata = null };
    }
...
pub const GpuAgentStats = struct {
    ...
    pub fn successRate(self: GpuAgentStats) f32 {
        ...
        return @as(f32, @floatFromInt(self.total_requests - self.failed_requests)) / @as(f32, @floatFromInt(self.total_requests));
    }
    pub fn gpuUtilizationRate(self: GpuAgentStats) f32 {
        if (self.total_requests == 0) return 0.0;
        return @as(f32, @floatFromInt(self.gpu_accelerated)) / @as(f32, @floatFromInt(self.total_requests));
    }
    pub fn avgTokensPerRequest(self: GpuAgentStats) f32 {
        if (self.total_requests == 0) return 0.0;
        return @as(f32, @floatFromInt(self.total_tokens)) / @as(f32, @floatFromInt(self.total_requests));
    }
};

pub const BackendInfo = struct { name: []const u8 = "cpu", device_count: u32 = 0, total_memory_mb: u64 = 0, available_memory_mb: u64 = 0, is_healthy: bool = false };
pub const LearningStatsInfo = struct { episodes: usize = 0, avg_episode_reward: f32 = 0, exploration_rate: f32 = 0, replay_buffer_size: usize = 0 };
...
pub const Agent = struct {
    ...
    pub const AgentStats = struct { history_length: usize, user_messages: usize, assistant_messages: usize, total_characters: usize, total_tokens_used: u64 };
    ...
    pub fn getStats(_: *const Agent) Agent.AgentStats {
        return .{ .history_length = 0, .user_messages = 0, .assistant_messages = 0, .total_characters = 0, .total_tokens_used = 0 };
    }
};
...
pub const GpuAgent = struct {
    ...
    pub fn initWithConfig(_: std.mem.Allocator, _: struct { default_timeout_ms: u64 = 30000, enable_learning: bool = true }) !*@This() {
        return error.FeatureDisabled;
    }
