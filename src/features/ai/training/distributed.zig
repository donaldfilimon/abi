...
pub const ParameterServer = struct {
    ...
    pub fn registerParameters(self: *ParameterServer, params: []const []const f32) !void {
        ...
        for (params) |param| {
            ...
            @memset(grad_zero, 0);
            try self.gradients.append(self.allocator, grad_zero);
        }
    }
...
/// Worker training loop with gradient accumulation
fn workerTrainingLoop(
    ...
    // Initialize gradient accumulation buffers (placeholder - would need actual gradient computation)
    // In a real implementation, these would store accumulated gradients across batches
    var accumulated_gradients: ?std.ArrayList(std.ArrayList(f32)) = null;
    defer if (accumulated_gradients) |*grads| {
        for (grads.items) |*grad| {
            grad.deinit();
        }
        grads.deinit();
    };

    var accumulation_step: usize = 0;
    ...
    while (epoch < epochs and worker.is_active) : (epoch += 1) {
        ...
        while (batch_start < local_data.len and worker.is_active) {
            ...
            const batch_size = 32; // Fixed batch size for accumulation
            const batch_end = @min(batch_start + batch_size, local_data.len);
            const batch_data = local_data[batch_start..batch_end];
            const batch_targets = local_targets[batch_start..batch_end];

            // Train on batch and accumulate gradients
            const loss = try trainBatchAndAccumulateGradients(worker.local_model, batch_data, batch_targets, &accumulated_gradients, trainer.allocator);
            ...
            // Synchronize with parameter server after accumulating enough gradients
            if (accumulation_step >= trainer.config.gradient_accumulation_steps) {
                // Convert accumulated gradients to the expected format
                var gradient_slices = std.ArrayList([]const f32){};
                defer gradient_slices.deinit();

                if (accumulated_gradients) |grads| {
                    for (grads.items) |grad| {
                        try gradient_slices.append(grad.items);
                    }
                }

                try synchronizeWithParameterServer(worker, trainer, gradient_slices.items);

                // Reset accumulation for next cycle
                if (accumulated_gradients) |*grads| {
                    for (grads.items) |*grad| {
                        grad.clearRetainingCapacity();
                    }
                }
                accumulation_step = 0;
            }
    ...
    // Final synchronization if there are accumulated gradients
...
/// Train on a batch and accumulate gradients
fn trainBatchAndAccumulateGradients(
    model: *anyopaque,
    batch_data: []const []const f32,
    batch_targets: []const []const f32,
    accumulated_gradients: *?std.ArrayList(std.ArrayList(f32)),
    allocator: std.mem.Allocator,
) !f32 {
    // Compute loss using existing function
    const loss = try trainBatch(model, batch_data, batch_targets);

    // Initialize gradient accumulation buffers if needed
    if (accumulated_gradients.* == null) {
        accumulated_gradients.* = std.ArrayList(std.ArrayList(f32)).init(allocator);
        // Initialize with dummy gradients based on input dimensions
        // In a real implementation, this would compute actual gradients from backward pass
        if (batch_data.len > 0) {
            const input_dim = batch_data[0].len;
            for (0..input_dim) |_| {
                var layer_grads = std.ArrayList(f32).init(allocator);
                // Dummy gradient computation - in reality this would be dL/dW
                try layer_grads.append(0.01 * @as(f32, @floatFromInt(batch_data.len)));
                try accumulated_gradients.*.?.append(layer_grads);
            }
        }
    } else {
        // Accumulate gradients onto existing buffers
        if (accumulated_gradients.*) |grads| {
            for (grads.items) |*layer_grad| {
                // Simulate gradient accumulation - in reality this would add computed gradients
                for (layer_grad.items) |*g| {
                    g.* += 0.01 * @as(f32, @floatFromInt(batch_data.len));
                }
            }
        }
    }
...
fn synchronizeWithParameterServer(worker: *DistributedTrainer.Worker, trainer: *DistributedTrainer, accumulated_gradients: ?[]const []const f32) !void {
    trainer.mutex.lock();
    defer trainer.mutex.unlock();

    // Push accumulated gradients to parameter server if provided
    if (accumulated_gradients) |gradients| {
        try trainer.parameter_server.pushGradients(worker.id, gradients);
    }
