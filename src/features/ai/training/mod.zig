...
pub fn calculateLearningRate(config: TrainingConfig, step: u64, base_lr: f32) f32 {
    return switch (config.learning_rate_schedule) {
        .constant => base_lr,
        .cosine => base_lr * 0.5 * (1 + @cos(@as(f32, @floatFromInt(step % config.decay_steps)) * 2 * std.math.pi / @as(f32, @floatFromInt(config.decay_steps)))),
        .warmup_cosine => {
            if (step < config.warmup_steps) {
                return base_lr * @as(f32, @floatFromInt(step)) / @as(f32, @floatFromInt(config.warmup_steps));
            }
            const adjusted_step = step - config.warmup_steps;
        ...
        .step => {
            const decay = @as(f32, @floatFromInt(step / config.decay_steps));
...
pub fn trainWithResult(
    ...
    var accumulator = try GradientAccumulator.init(allocator, model.gradients.len);
...
fn simulateGradient(
    ...
    const sample_factor: f32 = @as(f32, @floatFromInt(sample_count % 100)) * 0.0001;

    for (buffer, 0..) |*value, i| {
        const index_factor: f32 = @as(f32, @floatFromInt((i % 13) + 1)) * 0.0003;
        value.* = learning_rate + epoch_factor + batch_factor + sample_factor + index_factor;
        value.* *= @as(f32, @floatFromInt((i % 3) + 1));
    }
}
