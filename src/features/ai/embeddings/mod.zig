...
//!
//! // Use OpenAI backend
//! try ctx.useOpenAI("sk-...", "text-embedding-3-small");
//!
//! const embedding = try ctx.embed("Hello, world!");
...
pub const EmbeddingModel = struct {
    ...
    active_backend: ?EmbeddingBackend = null,
    ...
    pub fn setBackend(self: *EmbeddingModel, b: EmbeddingBackend) void {
        // Clean up old backend if present
        if (self.active_backend) |old| {
            old.deinit();
        }
    ...
    /// If a backend is set, uses it. Otherwise returns placeholder zeros.
    pub fn embed(self: *EmbeddingModel, text: []const u8) ![]f32 {
        // Use backend if available
        if (self.active_backend) |b| {
            const embedding = b.embed(self.allocator, text, self.config.dimension) catch |err| {
                // Convert backend error to module error
                return switch (err) {
                    BackendError.MissingCredentials => Error.EmbeddingFailed,
                    BackendError.RequestFailed => Error.EmbeddingFailed,
                    BackendError.InvalidResponse => Error.EmbeddingFailed,
                    BackendError.RateLimitExceeded => Error.EmbeddingFailed,
                    BackendError.OutOfMemory => error.OutOfMemory,
                    else => Error.EmbeddingFailed,
                };
            };
        ...
        // Fallback: return placeholder embedding (for backward compatibility)
    ...
    pub fn embedBatch(self: *EmbeddingModel, texts: []const []const u8) ![][]f32 {
        // Use backend batch if available
        if (self.active_backend) |b| {
            const embeddings = b.embedBatch(self.allocator, texts, self.config.dimension) catch |err| {
                return switch (err) {
                    BackendError.MissingCredentials => Error.EmbeddingFailed,
                    BackendError.RequestFailed => Error.EmbeddingFailed,
                    BackendError.InvalidResponse => Error.EmbeddingFailed,
    ...
    pub fn hasBackend(self: *const EmbeddingModel) bool {
        return self.active_backend != null;
    }

    /// Get the current backend type (if any).
    pub fn getBackendType(self: *const EmbeddingModel) ?BackendType {
        if (self.active_backend) |b| {
            return b.backend_type;
        }
        return null;
    }
};
...
pub const Context = struct {
    ...
    openai_backend: ?*backends.OpenAIBackend = null,
    ...
    pub fn init(allocator: std.mem.Allocator, cfg: config_module.EmbeddingsConfig) !*Context {
        ...
        const model = try allocator.create(EmbeddingModel);
        model.* = EmbeddingModel.init(allocator, .{
        ...
        // Initialize cache
        const cache = try allocator.create(EmbeddingCache);
        cache.* = EmbeddingCache.init(allocator);
        ctx.cache = cache;
    ...
    pub fn deinit(self: *Context) void {
        // Clean up owned backends
        if (self.openai_backend) |b| {
            b.deinit();
            // Note: deinit destroys itself, don't double-free
        }
        self.openai_backend = null;
    ...
    pub fn useOpenAI(self: *Context, api_key: []const u8, model_name: []const u8) !void {
        ...
        const oai = try backends.OpenAIBackend.init(self.allocator, api_key, model_name);
        self.openai_backend = oai;

        if (self.model) |m| {
            m.setBackend(oai.asBackend());
        }
    }
    ...
    pub fn useOpenAIFromEnv(self: *Context) !void {
        ...
        const oai = try backends.OpenAIBackend.initFromEnv(self.allocator);
        self.openai_backend = oai;
    ...
    pub fn embed(self: *Context, text: []const u8) ![]f32 {
        ...
        if (self.cache) |cache| {
            if (cache.get(text)) |cached| {
                ...
                return try self.allocator.dupe(f32, cached);
            }
        }

        // Generate new embedding
        if (self.model) |m| {
            const embedding = try m.embed(text);

            // Cache the result (store a copy)
            if (self.cache) |cache| {
                const cached_copy = try self.allocator.dupe(f32, embedding);
                cache.put(text, cached_copy) catch {
                    // Cache failure is not fatal, just log and continue
                    self.allocator.free(cached_copy);
                };
            }
    ...
    /// Check if a backend is configured.
    pub fn hasBackend(self: *const Context) bool {
        if (self.model) |m| {
            return m.hasBackend();
        }
        return false;
    ...
    /// Get the current backend type (if any).
    pub fn getBackendType(self: *const Context) ?BackendType {
        if (self.model) |m| {
            return m.getBackendType();
        }
        return null;
    }
};
