...
pub const LlamaWeights = struct {
    ...
    pub fn deinit(self: *LlamaWeights) void {
        ...
        for (self.layers) |_| {
            // Only free if we own these (not memory-mapped)
            // Complex cleanup omitted for brevity - weights are freed with gguf_file
        }
    ...
    pub fn loadFromGguf(self: *LlamaWeights, path: []const u8) !void {
        ...
        if (file.getTensorData("output.weight")) |data| {
            ...
            self.output_proj = try self.dequantizeTensor(data, info);
        }
