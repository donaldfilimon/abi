...
pub const PersonaBus = struct {
    ...
    const Inbox = struct {
        messages: std.BoundedArray(PersonaMessage, max_inbox_size) = .{},
    ...
    fn deliverTo(self: *Self, target: PersonaId, msg: PersonaMessage) void {
        ...
        inbox.messages.append(msg) catch {
    ...
    pub fn receive(self: *Self, id: PersonaId) ?PersonaMessage {
        ...
        if (inbox.messages.len == 0) return null;

        // Remove from front (shift)
        const msg = inbox.messages.buffer[0];
        // Shift remaining messages left
        for (1..inbox.messages.len) |i| {
            inbox.messages.buffer[i - 1] = inbox.messages.buffer[i];
        }
