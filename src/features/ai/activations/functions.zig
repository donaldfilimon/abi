...
test "Activation derivatives numerical verification" {
    const testing = std.testing;
    const epsilon = 1e-4;

    // Test points for numerical differentiation (avoid exact 0 for discontinuous functions)
    const test_points = [_]f32{ -2.0, -1.0, -0.5, -0.1, 0.01, 0.1, 0.5, 1.0, 2.0 };

    // Test each activation function
    inline for (.{ .leaky_relu, .sigmoid, .tanh, .softplus, .softsign, .linear }) |activation_type| {
        var processor = ActivationProcessor.init(.{ .activation_type = activation_type });

        for (test_points) |x| {
            // Compute analytical derivative
            const y = processor.activate(x);
            const analytical_deriv = processor.derivative(x, y);

            // Compute numerical derivative using central difference
            const y_plus = processor.activate(x + epsilon);
            const y_minus = processor.activate(x - epsilon);
            const numerical_deriv = (y_plus - y_minus) / (2.0 * epsilon);

            // Check if they match within tolerance
            const tolerance = if (@abs(numerical_deriv) > 1.0) @abs(numerical_deriv) * 1e-2 else 1e-3;
            try testing.expectApproxEqAbs(numerical_deriv, analytical_deriv, tolerance);
        }
    }

    // Test ReLU separately with special handling for discontinuity at 0
    var relu_processor = ActivationProcessor.init(.{ .activation_type = .relu });
    for (test_points) |x| {
        if (x == 0.01) continue; // Skip point too close to discontinuity

        const y = relu_processor.activate(x);
        const analytical_deriv = relu_processor.derivative(x, y);

        const y_plus = relu_processor.activate(x + epsilon);
        const y_minus = relu_processor.activate(x - epsilon);
        const numerical_deriv = (y_plus - y_minus) / (2.0 * epsilon);

        const tolerance = 1e-2; // More lenient for potential discontinuity effects
        try testing.expectApproxEqAbs(numerical_deriv, analytical_deriv, tolerance);
    }
}

test "Activation derivative edge cases" {
    const testing = std.testing;

    // Test ReLU derivative at 0 (should be 0 or 1 depending on convention)
    var relu_proc = ActivationProcessor.init(.{ .activation_type = .relu });
    const relu_deriv_at_zero = relu_proc.derivative(0.0, relu_proc.activate(0.0));
    try testing.expect(relu_deriv_at_zero >= 0.0 and relu_deriv_at_zero <= 1.0);

    // Test sigmoid derivative matches expected behavior
    var sigmoid_proc = ActivationProcessor.init(.{ .activation_type = .sigmoid });
    const x = 1.0;
    const y = sigmoid_proc.activate(x);
    const deriv = sigmoid_proc.derivative(x, y);
    const expected_deriv = y * (1.0 - y); // sigmoid'(x) = sigmoid(x) * (1 - sigmoid(x))
    try testing.expectApproxEqAbs(expected_deriv, deriv, 1e-6);

    // Test tanh derivative matches expected behavior
    var tanh_proc = ActivationProcessor.init(.{ .activation_type = .tanh });
    const tanh_x = 0.5;
    const tanh_y = tanh_proc.activate(tanh_x);
    const tanh_deriv = tanh_proc.derivative(tanh_x, tanh_y);
    const expected_tanh_deriv = 1.0 - tanh_y * tanh_y; // tanh'(x) = 1 - tanh(x)^2
    try testing.expectApproxEqAbs(expected_tanh_deriv, tanh_deriv, 1e-6);

    // Test linear derivative is always 1
    var linear_proc = ActivationProcessor.init(.{ .activation_type = .linear });
    try testing.expectEqual(@as(f32, 1.0), linear_proc.derivative(0.0, 0.0));
    try testing.expectEqual(@as(f32, 1.0), linear_proc.derivative(1.0, 1.0));
    try testing.expectEqual(@as(f32, 1.0), linear_proc.derivative(-1.0, -1.0));
}

test "Activation derivative batch computation" {
    const testing = std.testing;

    const input = [_]f32{ -1.0, 0.0, 1.0, 2.0 };
    var forward_output = [_]f32{0} ** 4;
    var derivative_output = [_]f32{0} ** 4;

    var processor = ActivationProcessor.init(.{ .activation_type = .relu });
    processor.activateBatch(&forward_output, &input);
    processor.derivativeBatch(&derivative_output, &input, &forward_output);

    // Verify each derivative matches individual computation
    for (input, forward_output, derivative_output) |x, y, d| {
        const expected_deriv = processor.derivative(x, y);
        try testing.expectApproxEqAbs(expected_deriv, d, 1e-6);
    }
}
