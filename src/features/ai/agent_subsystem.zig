...
/// This provides a clean abstraction for model execution without training concerns
pub const ModelInterface = struct {
    /// Context for the model implementation
    context: *anyopaque,
    /// Forward pass function
    forward_fn: *const fn (context: *anyopaque, input: []const f32, output: []f32) anyerror!void,
    /// Get input dimensions
    get_input_dims_fn: *const fn (context: *anyopaque) []const u32,
    /// Get output dimensions
    get_output_dims_fn: *const fn (context: *anyopaque) []const u32,
    /// Cleanup function
    deinit_fn: *const fn (context: *anyopaque) void,

    /// Execute forward pass
    pub fn forward(self: ModelInterface, input: []const f32, output: []f32) !void {
        return self.forward_fn(self.context, input, output);
    }

    /// Get input dimensions
    pub fn getInputDims(self: ModelInterface) []const u32 {
        return self.get_input_dims_fn(self.context);
    }

    /// Get output dimensions
    pub fn getOutputDims(self: ModelInterface) []const u32 {
        return self.get_output_dims_fn(self.context);
    }
...
/// Simple linear model implementation for basic inference
pub const LinearModel = struct {
    ...
    pub fn init(allocator: Allocator, input_size: u32, output_size: u32) !LinearModel {
        ...
        for (model.weights) |*w| w.* = (random.float(f32) - 0.5) * 0.1;
        for (model.biases) |*b| b.* = 0.0;

        return model;
    }

    pub fn deinit(self: *LinearModel) void {
        self.allocator.free(self.weights);
        self.allocator.free(self.biases);
    }

    pub fn forward(self: *LinearModel, input: []const f32, output: []f32) !void {
    ...
    /// Create a ModelInterface from this LinearModel
    pub fn asInterface(self: *LinearModel) ModelInterface {
        return ModelInterface{
            .context = self,
            .forward_fn = forwardCallback,
            .get_input_dims_fn = getInputDimsCallback,
            .get_output_dims_fn = getOutputDimsCallback,
            .deinit_fn = deinitCallback,
        };
    }
};

/// Legacy Model struct for backward compatibility - now wraps LinearModel
pub const Model = struct {
    interface: ModelInterface,

    pub fn init(allocator: Allocator, input_size: u32, output_size: u32) !*Model {
        var linear_model = try LinearModel.init(allocator, input_size, output_size);
        const model = try allocator.create(Model);
        model.* = .{ .interface = linear_model.asInterface() };
        return model;
    }

    pub fn deinit(self: *Model) void {
        self.interface.deinit();
        // Note: The interface deinit already handles cleanup, but we need to free the Model struct itself
        // This is a bit awkward in the legacy API - ideally this would be refactored
    }

    pub fn forward(self: *Model, input: []const f32, output: []f32) !void {
        return self.interface.forward(input, output);
    }

    pub fn setTraining(self: *Model, training: bool) void {
        _ = self;
        _ = training; // Inference-only interface doesn't need training mode
    }

    pub fn clearGradients(self: *Model) void {
        _ = self; // No gradients in inference-only interface
    }
};

// Callback functions for ModelInterface
fn forwardCallback(context: *anyopaque, input: []const f32, output: []f32) anyerror!void {
    const self = @as(*LinearModel, @ptrCast(@alignCast(context)));
    return self.forward(input, output);
}

fn getInputDimsCallback(context: *anyopaque) []const u32 {
    const self = @as(*const LinearModel, @ptrCast(@alignCast(context)));
    return &[_]u32{self.input_size};
}

fn getOutputDimsCallback(context: *anyopaque) []const u32 {
    const self = @as(*const LinearModel, @ptrCast(@alignCast(context)));
    return &[_]u32{self.output_size};
}

fn deinitCallback(context: *anyopaque) void {
    const self = @as(*LinearModel, @ptrCast(@alignCast(context)));
    self.deinit();
}
