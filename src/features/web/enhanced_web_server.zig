...
fn loginHandler(ctx: *RequestContext) !void {
    ...
    var parsed = std.json.parseFromSlice(LoginPayload, ctx.allocator, body, .{}) catch {
        ...
        try ctx.json(.{
            ...
            .message = "The JSON payload could not be parsed. Please ensure it is valid JSON with required fields: username, password",
            .code = "MALFORMED_JSON",
...
fn refreshTokenHandler(ctx: *RequestContext) !void {
    ...
    var parsed = std.json.parseFromSlice(RefreshPayload, ctx.allocator, body, .{}) catch {
        ...
        try ctx.json(.{
            .@"error" = "Malformed JSON in request body",
            .message = "The JSON payload could not be parsed. Please ensure it is valid JSON with required field: refresh_token",
            .code = "MALFORMED_JSON",
            .expected_format = .{ .refresh_token = "string" },
        }, .{});
        return;
    };
    defer parsed.deinit();
    ...
    if (parsed.value.refresh_token.len == 0) {
        ...
        try ctx.json(.{
            ...
            .message = "Refresh token is required and cannot be empty.",
