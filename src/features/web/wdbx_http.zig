...
pub const WdbxHttpServer = struct {
    ...
    fn handleHttpConnection(self: *WdbxHttpServer, connection: std.net.Server.Connection) !void {
        ...
        var response = self.respond(method, target, body_slice) catch |err| switch (err) {
            ...
            HttpError.MalformedJson => try self.buildErrorResponse(400, "malformed_json"),
            else => blk: {
    ...
    fn handleAgentQuery(self: *WdbxHttpServer, body: []const u8) !Response {
        ...
        const parsed = std.json.parseFromSlice(std.json.Value, self.allocator, body, .{}) catch {
            // Return structured JSON error for malformed JSON
            const error_response = .{
                .@"error" = "MALFORMED_JSON",
                .message = "The JSON payload could not be parsed. Please ensure it is valid JSON with required fields.",
                .code = "MALFORMED_JSON",
                .expected_format = .{
                    .query = "string (required)",
                    .provider = "string (optional, defaults to 'mock')",
                    .model = "string (optional)",
                    .max_tokens = "integer (optional, defaults to 256)",
                    .temperature = "number (optional, defaults to 0.2)",
                },
            };
            const json_text = try std.json.stringifyAlloc(self.allocator, error_response, .{});
            defer self.allocator.free(json_text);
            return Response{
                .status = 400,
                .body = try self.allocator.dupe(u8, json_text),
                .content_type = "application/json",
            };
        };
        defer parsed.deinit();

        const root_object = parsed.value.object orelse {
            // Return structured JSON error for invalid request structure
            const error_response = .{
        ...
        const query = root_object.get("query") orelse {
            const error_response = .{
                .@"error" = "MISSING_FIELD",
                .message = "Required field 'query' is missing from request.",
                .code = "MISSING_FIELD",
                .expected_format = .{
                    .query = "string (required)",
                    .provider = "string (optional)",
