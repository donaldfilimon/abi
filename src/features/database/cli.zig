...
pub const WdbxCLI = struct {
    ...
    pub fn run(self: *Self) !void {
        ...
        try self.validateCommandArguments();
    ...
    fn validateCommandArguments(self: *Self) !void {
        switch (self.options.command) {
            .help, .version, .windows => {
                // These commands don't require arguments
                return;
            },
            .add => {
                if (self.options.vector == null) {
                    try self.logger.err("Error: --vector is required for the 'add' command", .{});
                    try self.logger.info("Usage: wdbx add --vector \"1.0,2.0,3.0\" [--db <path>]", .{});
                    return error.MissingRequiredArgument;
                }
                if (self.options.db_path == null) {
                    try self.logger.warn("Warning: No database path specified with --db, using default", .{});
                }
                // Validate vector format
                if (self.options.vector) |vector_str| {
                    if (std.mem.indexOf(u8, vector_str, ",") == null) {
                        try self.logger.err("Error: Vector must be comma-separated floats (e.g., \"1.0,2.0,3.0\")", .{});
                        return error.InvalidVectorFormat;
                    }
                }
            },
            .query, .knn => {
                ...
                if (self.options.k == 0) {
                    try self.logger.err("Error: --k must be greater than 0", .{});
                    return error.InvalidArgument;
                }
                if (self.options.k > 1000) {
                    try self.logger.warn("Warning: Large k value ({}) may impact performance", .{self.options.k});
                }
            },
            .stats => {
                if (self.options.db_path == null) {
                    try self.logger.err("Error: --db is required for the 'stats' command", .{});
                    try self.logger.info("Usage: wdbx stats --db <path>", .{});
                    return error.MissingRequiredArgument;
                }
            },
            .server => {
                // Server command requires either --http or --tcp
                if (!std.mem.eql(u8, self.options.server_type, "http") and
                    !std.mem.eql(u8, self.options.server_type, "tcp"))
                {
                    try self.logger.err("Error: Server command requires --http or --tcp option", .{});
                    try self.logger.info("Usage: wdbx server --http [--port <port>] [--host <host>]", .{});
                    try self.logger.info("   or: wdbx server --tcp [--port <port>] [--host <host>]", .{});
                    return error.MissingRequiredArgument;
                }
            },
            .http, .tcp => {
                // Server commands are valid as-is, but validate port
                if (self.options.port < 1024 or self.options.port > 65535) {
                    try self.logger.err("Error: Port {} is outside valid range (1024-65535)", .{self.options.port});
                    return error.InvalidPort;
                }
            },
            .issue_credential => {
                if (self.options.role == null) {
                    try self.logger.err("Error: --role is required for the 'issue_credential' command", .{});
                    try self.logger.info("Usage: wdbx issue_credential --role <role_name>", .{});
                    return error.MissingRequiredArgument;
                }
            },
            .save, .load => {
            ...
            .tcp_test => {
                // Validate port for TCP test
                if (self.options.port < 1 or self.options.port > 65535) {
                    try self.logger.err("Error: Port {} is outside valid range (1-65535)", .{self.options.port});
                    return error.InvalidPort;
                }
            },
        }
        ...
        if (self.options.host) |host| {
            if (host.len == 0) {
                try self.logger.err("Error: Host cannot be empty", .{});
    ...
    while (arg_index < args.len) {
        ...
        if (std.mem.eql(u8, arg, "--db")) {
            ...
            if (arg_index < args.len) {
                ...
                if (options.k == 0) {
                    std.debug.print("Error: --k must be greater than 0\n", .{});
                    std.process.exit(1);
                }
            } else {
                std.debug.print("Error: --k requires a value\n", .{});
                std.process.exit(1);
            }
            ...
            if (arg_index < args.len) {
                ...
                if (options.port == 0) {
                    std.debug.print("Error: Port cannot be 0\n", .{});
                    std.process.exit(1);
                }
            } else {
