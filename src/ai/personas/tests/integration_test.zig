...
test "Multi-Persona System End-to-End Workflow" {
    ...
    // 2. Initialize the high-level system orchestrator
    var system = try personas.MultiPersonaSystem.init(allocator, cfg);
    defer system.deinit();

    // 3. Register and configure specialized personas
    // We register Abbey (Empathetic), Aviva (Expert), and Abi (Moderator)

    // Abbey Initialization
    var abbey_persona = try personas.abbey.AbbeyPersona.init(allocator, cfg.abbey);
    errdefer abbey_persona.deinit();
    try system.ctx.registerPersona(.abbey, abbey_persona.interface());

    // Aviva Initialization
    var aviva_persona = try personas.aviva.AvivaPersona.init(allocator, cfg.aviva);
    errdefer aviva_persona.deinit();
    try system.ctx.registerPersona(.aviva, aviva_persona.interface());

    // Abi Persona Initialization (The router itself can also act as a persona)
    var abi_persona = personas.abi.AbiPersona.init(system.router);
    try system.ctx.registerPersona(.abi, abi_persona.interface());

    // 4. Test Scenario A: Empathetic Routing
