...
pub const VectorStoreError = error{
    DimensionMismatch,
};
...
pub const VectorStore = struct {
    ...
    pub fn deinit(self: *VectorStore) void {
        for (self.records.items) |record| {
            self.allocator.free(record.values);
    ...
    pub fn insert(self: *VectorStore, values: []const f32, metadata: ?[]const u8) !u64 {
    ...
    pub fn search(self: *VectorStore, allocator: std.mem.Allocator, query: []const f32, k: usize) ![]SearchResult {
        if (self.dimension == null or self.records.items.len == 0) {
            return allocator.alloc(SearchResult, 0);
        }
        if (query.len != self.dimension.?) {
            return VectorStoreError.DimensionMismatch;
        }
        const total = self.records.items.len;
        var temp = try allocator.alloc(SearchResult, total);
        errdefer allocator.free(temp);
        for (self.records.items, 0..) |record, idx| {
            temp[idx] = .{
                .id = record.id,
                .distance = distanceSquared(query, record.values),
                .metadata = if (record.metadata) |meta| meta else null,
            };
        }
        insertionSort(temp);
...
fn distanceSquared(a: []const f32, b: []const f32) f32 {
    var sum: f64 = 0;
    for (a, b) |lhs, rhs| {
        const diff = @as(f64, lhs) - @as(f64, rhs);
        sum += diff * diff;
    }
    return @floatCast(sum);
}
fn insertionSort(slice: []SearchResult) void {
    var i: usize = 1;
    while (i < slice.len) : (i += 1) {
        const key = slice[i];
        var j = i;
        while (j > 0 and key.distance < slice[j - 1].distance) : (j -= 1) {
            slice[j] = slice[j - 1];
        }
        slice[j] = key;
    }
...
pub const RateLimiter = struct {
    max_actions: usize,
    actions: usize = 0,
    pub fn init(max_actions: usize) RateLimiter {
        return .{ .max_actions = max_actions };
    }
    pub fn tryConsume(self: *RateLimiter) bool {
        if (self.actions >= self.max_actions) return false;
        self.actions += 1;
        return true;
    }
};
pub const State = struct {
    allocator: std.mem.Allocator,
    framework: framework_runtime.Framework,
    vector_store: VectorStore,
    rate_limiter: RateLimiter,
    pub fn init(allocator: std.mem.Allocator) !State {
        var framework = try framework_runtime.Framework.init(allocator, .{});
        errdefer framework.deinit();
        return State{
            .allocator = allocator,
            .framework = framework,
            .vector_store = VectorStore.init(allocator),
            .rate_limiter = RateLimiter.init(128),
        };
    }
    ...
    pub fn consumeBudget(self: *State) !void {
        if (!self.rate_limiter.tryConsume()) {
            return errors.CommandError.RateLimited;
        }
    }
};
pub fn allFeatures() []const framework_config.Feature {
    return &std.meta.tags(framework_config.Feature) ** 1;
}
