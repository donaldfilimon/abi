...
pub const commands = [_]Command{
    ...
    .{ .name = "complete", .usage = "abi complete [--live] [--confirm] [--learn] [--model <id>] <input>", .summary = "Run local model completion with WDBX metadata; --model selects a catalog id (e.g. claude-fable-5); --live serves anthropic models over the live transport; --learn runs the SEA self-learning loop (evidence recall + router adaptation); --model apple-fm --confirm targets on-device FoundationModels" },
