...
pub const HealthChecker = struct {
    ...
    pub fn exportHealthStatus(self: *Self, allocator: std.mem.Allocator) ![]const u8 {
        ...
        var checks_json = std.ArrayList(u8).init(allocator);
        defer checks_json.deinit();

        try checks_json.appendSlice("[");
        for (health.checks.items, 0..) |check, i| {
            if (i > 0) try checks_json.appendSlice(",");

            // Build metadata JSON
            var metadata_json = std.ArrayList(u8).init(allocator);
            defer metadata_json.deinit();

            try metadata_json.appendSlice("{");
            var iter = check.metadata.iterator();
            var first = true;
            while (iter.next()) |entry| {
                if (!first) try metadata_json.appendSlice(",");
                try metadata_json.writer().print("\"{s}\":\"{s}\"", .{ entry.key_ptr.*, entry.value_ptr.* });
                first = false;
            }
            try metadata_json.appendSlice("}");

            try checks_json.writer().print("{{\"name\":\"{s}\",\"status\":\"{s}\",\"message\":\"{s}\",\"timestamp\":{d},\"response_time_ms\":{d},\"metadata\":{s}}}", .{ check.name, check.status.toString(), check.message, check.timestamp, check.response_time_ms, metadata_json.items });
        }
        try checks_json.appendSlice("]");
