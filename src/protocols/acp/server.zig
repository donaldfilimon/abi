...
pub const Server = struct {
    ...
    pub fn createSession(self: *Server, metadata: ?[]const u8) ![]const u8 {
        ...
        try self.sessions.put(self.allocator, id, session);
        return id;
    }
...
fn handleSessionsHttpRoute(
    ...
    if (std.mem.eql(u8, path, "/sessions")) {
        ...
        defer allocator.free(body);
        const metadata = acpExtractMessage(allocator, body) catch null;
    ...
    if (std.mem.startsWith(u8, path, "/sessions/")) {
        ...
        if (remainder.len == 0) return acpRespondJson(request, "{\"error\":\"not found\"}", .not_found);
        ...
        if (std.mem.indexOf(u8, remainder, "/tasks")) |idx| {
            ...
            if (request.head.method == .POST) {
                ...
                defer allocator.free(body);
                const task_id = acpExtractMessage(allocator, body) catch body;
                ...
                acp_server.addTaskToSession(session_id, task_id) catch |err| switch (err) {
                    error.SessionNotFound => {
                        return acpRespondJson(request, "{\"error\":\"session not found\"}", .not_found);
                    },
                    else => return err,
                };
                return acpRespondJson(request, "{\"ok\":true}", .ok);
            }
            return acpRespondJson(request, "{\"error\":\"method not allowed\"}", .method_not_allowed);
        }
...
test "Session toJson" {
    ...
    const session_id = try server.createSession("test-meta");
    const session = server.getSession(session_id).?;
