...
test "auth: null token allows all requests" {
    const allocator = std.testing.allocator;
    var server = Server.init(allocator, "test", "0.1.0");
    defer server.deinit();
    // auth_token defaults to null — no auth required
...
test "auth: rejects tools/list without token when auth enabled" {
    const allocator = std.testing.allocator;
    var server = Server.init(allocator, "test", "0.1.0");
    defer server.deinit();
    ...
    const written = out[0..writer.end];
    try std.testing.expect(std.mem.indexOf(u8, written, "-32001") != null);
    try std.testing.expect(std.mem.indexOf(u8, written, "Unauthorized") != null);
}

test "auth: rejects tools/call without token when auth enabled" {
    const allocator = std.testing.allocator;
    var server = Server.init(allocator, "test", "0.1.0");
    defer server.deinit();
    server.auth_token = "secret-token-123";
    ...
    try server.addTool(.{
        .def = .{ .name = "echo", .description = "Echo", .input_schema = "{}" },
        .handler = struct {
            fn handle(_: std.mem.Allocator, _: ?std.json.ObjectMap, o: *std.ArrayListUnmanaged(u8)) !void {
                try o.appendSlice(std.testing.allocator, "ok");
            }
...
test "auth: rejects wrong token" {
    ...
    try server.processMessage(
        \\{"jsonrpc":"2.0","method":"tools/list","id":1,"params":{"_auth_token":"wrong-token"}}
    , &writer);
...
test "auth: accepts correct token for tools/list" {
    const allocator = std.testing.allocator;
    var server = Server.init(allocator, "test", "0.1.0");
    defer server.deinit();
    server.auth_token = "correct-token";
    ...
    try server.processMessage(
        \\{"jsonrpc":"2.0","method":"tools/list","id":1,"params":{"_auth_token":"correct-token"}}
    , &writer);
...
test "auth: accepts correct token for tools/call" {
    const allocator = std.testing.allocator;
    var server = Server.init(allocator, "test", "0.1.0");
    defer server.deinit();
    server.auth_token = "my-secret";

    try server.addTool(.{
        .def = .{ .name = "greet", .description = "Greeting", .input_schema = "{}" },
        .handler = struct {
            fn handle(_: std.mem.Allocator, _: ?std.json.ObjectMap, o: *std.ArrayListUnmanaged(u8)) !void {
                try o.appendSlice(std.testing.allocator, "Hello!");
            }
        }.handle,
    });
    ...
    try server.processMessage(
        \\{"jsonrpc":"2.0","method":"tools/call","id":3,"params":{"name":"greet","_auth_token":"my-secret"}}
    , &writer);
...
test "auth: ping allowed without token when auth enabled" {
    const allocator = std.testing.allocator;
    var server = Server.init(allocator, "test", "0.1.0");
    defer server.deinit();
...
test "auth: initialize allowed without token when auth enabled" {
    const allocator = std.testing.allocator;
    var server = Server.init(allocator, "test", "0.1.0");
    defer server.deinit();
    server.auth_token = "secret";
...
test "auth: rejects resources/list without token" {
    const allocator = std.testing.allocator;
    var server = Server.init(allocator, "test", "0.1.0");
    defer server.deinit();
    server.auth_token = "secret";
...
test "auth: rejects resources/read without token" {
    const allocator = std.testing.allocator;
    var server = Server.init(allocator, "test", "0.1.0");
    defer server.deinit();
    server.auth_token = "secret";
    ...
    try server.processMessage(
        \\{"jsonrpc":"2.0","method":"resources/read","id":1,"params":{"uri":"abi://test"}}
    , &writer);
