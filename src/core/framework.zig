...
test "Framework initializes registry with enabled features" {
    // Test minimal framework - no config-driven features enabled.
    // Configless features (compute, documents, desktop) are still registered
    // unconditionally when their compile-time flags are on.
    var fw = try Framework.initMinimal(std.testing.allocator);
    defer fw.deinit();

    // Registry should be initialized with only configless features
    const configless_count: usize = comptime (if (build_options.feat_compute) @as(usize, 1) else 0) +
        (if (build_options.feat_documents) @as(usize, 1) else 0) +
        (if (build_options.feat_desktop) @as(usize, 1) else 0);
    try std.testing.expectEqual(configless_count, fw.registry.count());
...
test "Framework.getRegistry returns mutable registry" {
    ...
    const configless_count: usize = comptime (if (build_options.feat_compute) @as(usize, 1) else 0) +
        (if (build_options.feat_documents) @as(usize, 1) else 0) +
        (if (build_options.feat_desktop) @as(usize, 1) else 0);
    try std.testing.expectEqual(configless_count, reg.count());
}
...
test "Framework.listRegisteredFeatures returns empty for minimal" {
    ...
    const configless_count: usize = comptime (if (build_options.feat_compute) @as(usize, 1) else 0) +
        (if (build_options.feat_documents) @as(usize, 1) else 0) +
        (if (build_options.feat_desktop) @as(usize, 1) else 0);
    try std.testing.expectEqual(configless_count, features.len);
}
