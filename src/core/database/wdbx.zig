...
pub fn insertVector(
    ...
    if (vector.len == 0 and handle.db.records.items.len > 0) {
        const dimension = handle.db.records.items[0].vector.len;
        if (dimension > 0) {
            const owned_vector = if (handle.db.vector_pool) |pool|
                try pool.alloc(dimension)
            else
                try handle.db.allocator.alloc(f32, dimension);
            @memset(owned_vector, 0);

            const metadata_copy = if (metadata) |meta|
                handle.db.allocator.dupe(u8, meta) catch |err| {
                    if (handle.db.vector_pool) |pool| {
                        pool.free(owned_vector);
                    } else {
                        handle.db.allocator.free(owned_vector);
                    }
                    return err;
                }
            else
            ...
            try handle.db.insertOwned(id, owned_vector, metadata_copy);
            return;
        }
    }
