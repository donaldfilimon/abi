...
test "c_api: simd dot product" {
    ...
    const result = abi.foundation.simd.vectorDot(&a, &b);
...
test "c_api: simd cosine similarity" {
    ...
    const result = abi.foundation.simd.cosineSimilarity(&a, &b);
    ...
    const result2 = abi.foundation.simd.cosineSimilarity(&c, &d);
...
test "c_api: simd L2 norm" {
    ...
    const result = abi.foundation.simd.vectorL2Norm(&v);
