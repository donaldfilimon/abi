...
test "json escape basic strings" {
    ...
    const escaped = try abi.foundation.utils.json.escapeJsonContent(allocator, "hello world");
...
test "json escape special characters" {
    ...
    const escaped = try abi.foundation.utils.json.escapeJsonContent(allocator, "line1\nline2\ttab\"quote\\backslash");
    defer allocator.free(escaped);
...
test "json escape with quotes" {
    ...
    const escaped = try abi.foundation.utils.json.escapeString(allocator, "hello");
    defer allocator.free(escaped);
...
test "json parse string field" {
    ...
    const object = try abi.foundation.utils.json.getRequiredObject(parsed.value);
    const name = try abi.foundation.utils.json.parseStringField(object, "name", allocator);
    defer allocator.free(name);
    try std.testing.expectEqualStrings("test", name);

    const value = try abi.foundation.utils.json.parseIntField(object, "value");
...
test "json parse optional fields" {
    ...
    const required = try abi.foundation.utils.json.parseStringField(object, "required", allocator);
    defer allocator.free(required);
    try std.testing.expectEqualStrings("present", required);

    const missing = abi.foundation.utils.json.parseOptionalStringField(object, "missing", allocator) catch null;
...
test "json parse bool field" {
    ...
    const enabled = try abi.foundation.utils.json.parseBoolField(object, "enabled");
    try std.testing.expect(enabled);

    const disabled = try abi.foundation.utils.json.parseBoolField(object, "disabled");
...
test "json parse number fields" {
    ...
    const int_val = try abi.foundation.utils.json.parseIntField(object, "int_val");
    try std.testing.expectEqual(@as(i64, 123), int_val);

    const float_val = try abi.foundation.utils.json.parseNumberField(object, "float_val");
