...
description: Plan and execute ABI repo work from current TODOs, roadmap/spec docs, nightly Rust constraints, and validation gates. Use when user asks to find all ABI todos/roadmaps, compile ABI goals/specs, organize a large ABI implementation goal, or decide the next safe work slice in ~/dev/active/abi.
---
...
4. Optional: refresh the inventory with the abi-mega `abi_inventory` Python helper (`--repo /Users/donaldfilimon/dev/active/abi`) if available (ships with the codex `abi-mega` plugin, not this repo); skip when absent.
...
#   abi_inventory --repo /Users/donaldfilimon/dev/active/abi
