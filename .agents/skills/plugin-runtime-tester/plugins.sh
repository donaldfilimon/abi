...
printf '%s' "$list" | grep -qF "Installed Plugins (" && echo "[ok] registry lists plugins" \
...
for p in telemetry-exporter ai-plugin hash-plugin sea-plugin foundationmodels-plugin; do
    ...
    if printf '%s' "$out" | grep -qF "event (bytes="; then
        echo "[ok] run $p -> $out"
    else
...
printf '%s' "$out" | grep -qiF "PluginNotFound" && echo "[ok] unknown -> PluginNotFound" \
