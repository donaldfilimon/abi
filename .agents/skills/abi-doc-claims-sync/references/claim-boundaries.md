...
- ABI is a Rust nightly local AI orchestration framework with deterministic Abbey/Aviva/Abi profile routing.
...
- GPU support includes capability/status reporting and vector operations with
  linked Metal DOT on macOS when the `metal-kernels` feature initializes,
  else deterministic CPU SIMD fallback (`accelerated=false` when kernels are
  not linked).
...
- Native local CUDA/Vulkan/ANE execution when the source only proves detection, reporting, or fallback (macOS Metal DOT is real when initialized — still not a blanket GPU speedup claim).
...
- Nightly Rust via `rust-toolchain.toml`; always use `./tools/cargo.sh` (never bare `cargo`).
- `./tools/check.sh`, docs validation, and cross-smoke guidance.
