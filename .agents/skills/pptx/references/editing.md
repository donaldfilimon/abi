...
```bash
python $SKILL_DIR/scripts/office/unpack.py template.pptx unpacked/
python $SKILL_DIR/scripts/inspect_slide.py unpacked/ --summary --theme --media   # compact overview
python $SKILL_DIR/scripts/inspect_slide.py unpacked/ppt/slides/slide1.xml       # detail for slides you keep
# Delete unwanted slides (by number or --keep the ones you want)
python $SKILL_DIR/scripts/delete_slide.py unpacked/ --keep 1 4 8
# Replace text in EVERY kept slide (see below for details)
python $SKILL_DIR/scripts/replace_text.py unpacked/ppt/slides/slide1.xml --ph ctrTitle --text "New Title"
# If duplicate text (e.g., multiple "Chapter Title" bullets), use replace_nth_text.py:
python $SKILL_DIR/scripts/replace_nth_text.py unpacked/ppt/slides/slide4.xml --find "Chapter Title" --nth 1 --text "2025 VISION"

# ── VERIFY: no template text remains ──
python $SKILL_DIR/scripts/replace_nth_text.py unpacked/ppt/slides/slide1.xml --list
python $SKILL_DIR/scripts/replace_nth_text.py unpacked/ppt/slides/slide2.xml --list
...
python $SKILL_DIR/scripts/check_overlaps.py unpacked/ --fix
python $SKILL_DIR/scripts/clean.py unpacked/
python $SKILL_DIR/scripts/office/pack.py unpacked/ output.pptx --original template.pptx
```
...
1. **Unpack**: `python $SKILL_DIR/scripts/office/unpack.py template.pptx unpacked/`
...
2. **Get overview** — use `--summary` to avoid output truncation on large templates:
   ...
   python $SKILL_DIR/scripts/inspect_slide.py unpacked/ --summary --theme --media
   ...
   python $SKILL_DIR/scripts/media_grid.py unpacked/

   # Then inspect specific slides you plan to keep (detailed per-run formatting)
   python $SKILL_DIR/scripts/inspect_slide.py unpacked/ppt/slides/slide1.xml
   ```
...
4. **Build presentation** (do this yourself, not with subagents):
   ...
   python $SKILL_DIR/scripts/delete_slide.py unpacked/ 3 5 6 7 9 10
   # Or equivalently: keep only slides 1, 2, 4, 8
   python $SKILL_DIR/scripts/delete_slide.py unpacked/ --keep 1 2 4 8

   # Duplicate a slide you want to reuse (auto-inserts into presentation order)
   python $SKILL_DIR/scripts/add_slide.py unpacked/ slide2.xml                  # append at end
   python $SKILL_DIR/scripts/add_slide.py unpacked/ slide2.xml --after slide4.xml  # insert after slide4
   ```
...
5. **Replace placeholder text** — use `replace_text.py` instead of manual XML editing:
   ...
   python $SKILL_DIR/scripts/replace_text.py unpacked/ppt/slides/slide1.xml --ph subTitle --text "Line 1\nLine 2"

   # Custom formatting per run (when you need different fonts/colors/sizes)
   python $SKILL_DIR/scripts/replace_text.py unpacked/ppt/slides/slide1.xml --ph ctrTitle --runs '[
   ...
   python $SKILL_DIR/scripts/replace_text.py unpacked/ppt/slides/slide1.xml --match "Christmas" --text "Holiday Special"
   ```
...
python $SKILL_DIR/scripts/inspect_slide.py unpacked/ --theme --media
...
# Custom columns
python $SKILL_DIR/scripts/media_grid.py unpacked/ assets --cols 6
```
...
# Multi-line
python $SKILL_DIR/scripts/replace_text.py unpacked/ppt/slides/slide1.xml --ph ctrTitle --text "Line 1\nLine 2"
...
python $SKILL_DIR/scripts/replace_text.py unpacked/ppt/slides/slide1.xml --match "Christmas" --text "Holiday"

# Dry run (preview without modifying)
python $SKILL_DIR/scripts/replace_text.py unpacked/ppt/slides/slide1.xml --ph ctrTitle --text "New" --dry-run
```
...
python $SKILL_DIR/scripts/resize_shape.py unpacked/ppt/slides/slide1.xml --list

# Set absolute height (inches) — text box too short for new content
python $SKILL_DIR/scripts/resize_shape.py slide.xml --ph body --height 3.5

# Set absolute width and height
python $SKILL_DIR/scripts/resize_shape.py slide.xml --ph ctrTitle --width 9.0 --height 2.0

# Grow taller by 0.5" relative to current size
python $SKILL_DIR/scripts/resize_shape.py slide.xml --ph body --dh 0.5

# Move a shape down by 0.3" to make room above
python $SKILL_DIR/scripts/resize_shape.py slide.xml --ph body --dy 0.3

# Match by shape name or text content
python $SKILL_DIR/scripts/resize_shape.py slide.xml --name "Title 1" --height 2.0
python $SKILL_DIR/scripts/resize_shape.py slide.xml --match "Lorem" --height 4.0

# Dry run (preview without modifying)
python $SKILL_DIR/scripts/resize_shape.py slide.xml --ph body --dh 0.5 --dry-run
```
...
python $SKILL_DIR/scripts/delete_slide.py unpacked/ slide3.xml slide5.xml slide7.xml
python $SKILL_DIR/scripts/delete_slide.py unpacked/ 3 5 7

# Keep only specific slides (delete everything else) — usually easier
python $SKILL_DIR/scripts/delete_slide.py unpacked/ --keep 1 4 8

# List all slides in presentation order
python $SKILL_DIR/scripts/delete_slide.py unpacked/ --list

# Dry run (preview without modifying)
python $SKILL_DIR/scripts/delete_slide.py unpacked/ --keep 1 4 8 --dry-run
```
...
python $SKILL_DIR/scripts/add_slide.py unpacked/ slide2.xml --after slide1.xml
python $SKILL_DIR/scripts/add_slide.py unpacked/ slide2.xml --after 1
...
```bash
python $SKILL_DIR/scripts/delete_slide.py unpacked/ --keep 1 4 8   # keep only these
python $SKILL_DIR/scripts/delete_slide.py unpacked/ 3 5 7           # delete these
```
...
1. **Use `--autofit`** when replacing text — adds `<a:normAutofit/>` to the text box, which makes PowerPoint auto-shrink the font to fit:
   ```bash
   python $SKILL_DIR/scripts/replace_text.py slide.xml --ph ctrTitle --text "A Very Long Title" --autofit
   ```
...
2. **Resize the text box** with `resize_shape.py` — make it taller or wider to fit longer content:
   ...
   python $SKILL_DIR/scripts/resize_shape.py slide.xml --ph body --height 4.0

   # If resizing causes overlap, move the shape below it down
   python $SKILL_DIR/scripts/resize_shape.py slide.xml --name "Footer Text" --dy 0.5
   ```

3. **Reduce font size explicitly** in `--runs` when replacement is significantly longer than original:
   ```bash
   python $SKILL_DIR/scripts/replace_text.py slide.xml --ph body --runs '[
...
python $SKILL_DIR/scripts/replace_nth_text.py slide2.xml --find "YYYY" --all --text "2015"
python $SKILL_DIR/scripts/replace_nth_text.py slide2.xml --find "[Fan Club Name]" --all --text "ONCE"
python $SKILL_DIR/scripts/replace_nth_text.py slide2.xml --find "[Company Name]" --all --text "JYP Entertainment"
```
...
python $SKILL_DIR/scripts/replace_nth_text.py slide4.xml --list --find "Chapter Title"
#  Idx   Line  Text
#  ---   ----  ----
#    3    132  Chapter Title
#    4    176  Chapter Title
...
python $SKILL_DIR/scripts/replace_nth_text.py slide4.xml --find "Chapter Title" --nth 1 --text "2025 CREATIVE VISION" --full
python $SKILL_DIR/scripts/replace_nth_text.py slide4.xml --find "Chapter Title" --nth 1 --text "CIRCULARITY METRICS" --full
# Note: after replacing the 1st, the former 2nd becomes the new 1st

# Or target by absolute index (0-based, from --list output)
python $SKILL_DIR/scripts/replace_nth_text.py slide4.xml --index 3 --text "2025 CREATIVE VISION"
python $SKILL_DIR/scripts/replace_nth_text.py slide4.xml --index 4 --text "CIRCULARITY METRICS"
```
