...
**Status:** **CURRENT — all five designs ported to ABI Zig source with passing tests.** IoT stream monitoring (`src/features/ai/iot_monitor.zig`), multimodal fusion (`src/features/ai/multimodal_fusion.zig`), the trainable `PointNeuralNetwork` + `SoulLayout` (`src/features/ai/point_neural_net.zig` + `src/features/ai/soul_layout.zig`), the per-turn `PipelineTelemetry` snapshot (`src/features/ai/pipeline_telemetry.zig`), and the Discord gateway loop (`src/connectors/discord_gateway.zig`) are all landed. The Discord gateway's WebSocket client performs the real handshake + masked frames but TLS is not linked (deploy behind a TLS-terminating proxy for `wss`); offline transports drive the loop in tests.
...
## 8. Cross-reference: ABI today vs. WDBX (Rust, updated 2026-07-15)
...
| --- | --- | --- |
| **Governance E-score** | 4-pillar weighted scalar `E = 0.25·Aut + 0.35·NonMal + 0.20·Ben + 0.20·Just`, per-turn, with a hard safety veto; surfaced in `PipelineTelemetry.ethical_scores` (`src/features/ai/constitution`, `src/pipeline.rs`). | `feat-ai` maps that lineage onto ABI's six-principle constitution: `AuditResult` carries per-principle scores, weighted `escore`, and `vetoed`; CLI/MCP completion output exposes `audit_escore`/`audit_vetoed`; persisted completion metadata stores `escore` and `audit_vetoed`. The mandatory per-turn `PipelineTelemetry.ethical_scores` snapshot now exists (`src/features/ai/pipeline_telemetry.zig`), folding `AuditResult.scores` into the snapshot's `ethical_scores` field. |
| **Telemetry emission discipline** | Mandatory `PipelineTelemetry` snapshot every turn (provider, retrieval, neural, cluster, ethics, p99, errors); repo rule ties code changes to telemetry updates. | ABI Zig `PipelineTelemetry` (`src/features/ai/pipeline_telemetry.zig`) is the single mandatory per-turn snapshot: `ObservabilityHub.recordResponse`/`recordError`/`recordConstitutionBlock` aggregate counters and `finishTurn`/`snapshot` folds the `AuditResult` ethics scores, `NeuralTelemetry` (from `PointNeuralNetwork.telemetry`), provider/retrieval/cluster/governance/guardrail summaries, and p99/error counts into one owned struct freed via `PipelineTelemetry.deinit`. Counters are fire-and-forget; the snapshot is the mandatory tail. |
| **Trainable neural net** | Real `PointNeuralNetwork` MLP (`[3,8,1]`) with genuine backprop/SGD (`backtrace_teach`), topology optimization + weight pruning, serde persistence, and weight telemetry (`src/neural/`). | ABI Zig `PointNeuralNetwork` (`src/features/ai/point_neural_net.zig`) implements a real trainable MLP with `forwardWithTrace`/`backtraceTeach` backprop+SGD, `optimizeTopology` (weight pruning + dead-neuron removal), JSON `save`/load, and `NeuralTelemetry`. It is now invoked by `training.zig`: when a dataset is available, `train` builds a `[3,8,3]` autoencoder, trains it on the records for 50 epochs, runs topology optimization, and persists weights to `artifact_dir/neural_net_<profile>.json`, reporting `model trained on N records` (falls back to the metadata-only disclosure when no dataset is present). |
...
| **Discord gateway** | Working WS gateway loop (Hello/Identify/heartbeat/sequence) + REST reply POST, prefix-scoped + bot-safe + token-safe routing (`src/features/discord.rs`). | ABI Zig `DiscordGateway` (`src/connectors/discord_gateway.zig`) implements the same surface: `GATEWAY_URL`/`API_BASE`/`DEFAULT_INTENTS` constants, a `Gateway.run` loop over Hello→Identify→heartbeat→MESSAGE_CREATE (op 10/2/1/0), `parseDiscordCommand`/`routeDiscordMessage`/`truncate` (prefix-scoped, bot-safe, token-safe), and a `WebSocketClient` doing the real handshake + masked frames (TLS not linked — deploy behind a proxy). Offline `FakeTransport` drives the loop in tests; 5 tests pass. No autonomous token-bearing POST is issued without a live transport. |
...
All five designs were **promoted to Current**:
 ...
 - **Trainable `PointNeuralNetwork` + `SoulLayout`** (`src/features/ai/point_neural_net.zig`
   ...
   ~20 tests, mod/stub parity. Now wired into the `training.zig` train path (real autoencoder training + weight persistence when a dataset is available).
 - **Per-turn `PipelineTelemetry` snapshot** (`src/features/ai/pipeline_telemetry.zig`) —
   `ObservabilityHub` aggregates response/error/constitution-block counters and `snapshot`
   folds the `AuditResult` ethics scores, `NeuralTelemetry`, provider/retrieval/cluster/
   governance/guardrail summaries, and p99/error counts into one owned struct; 3 tests.
 - **Discord gateway loop** (`src/connectors/discord_gateway.zig`) — WS-gateway connect →
   Identify → heartbeat → MESSAGE_CREATE loop with prefix-scoped, bot-safe, token-safe
   command routing and masked-frame `WebSocketClient`; offline `FakeTransport` drives 5 tests.
   TLS is not linked, so `wss` must sit behind a TLS-terminating proxy; no token-bearing POST
   fires without a live transport.

No WDBX-unique designs remain unadopted. Any further promotion must land as real ABI source +
tests under the Mod/Stub parity rules, pass `./build.sh check`, keep the
`tests/contracts/public_docs.zig` claim boundary green, and be tracked in `tasks/`.
