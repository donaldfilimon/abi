...
| `src/mcp/main.zig`, `src/mcp/handlers.zig`, `src/mcp/stdio_transport.zig`, `src/mcp/http_transport.zig`, `src/mcp/server.zig` | MCP runtime, tool catalog, stdio transport, loopback HTTP/SSE transport, and server re-export facade. |
...
- native CUDA / Vulkan / ANE dispatch, or blanket GPU acceleration beyond the macOS Metal fused cosine/dot/L2 parts path when kernels initialize;
