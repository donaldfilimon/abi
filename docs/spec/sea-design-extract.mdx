...
description: "Status: PARTIAL / DESIGN REFERENCE — core types, scorer, and selection ported to ABI Rust source."
---

**Date:** 2026-06-27 (updated 2026-07-06)
**Status:** **PARTIAL / DESIGN REFERENCE — MemoryKind, Authority, the 8-signal scorer, cluster-diversity selection, and context packing are implemented in ABI Rust (`crates/abi-sea/src/`). The remaining Proposed items (production performance tuning and learned codec integration) remain design reference.**
...
> **Disclaimer (standard).** Design-reference extract only. Where this document and ABI source disagree, ABI source wins (`Cargo.toml`, `crates/`, `tests/golden/`, `./tools/check.sh`). Public claim boundaries: `docs/contracts/external-claims-audit.mdx`.
>
...
> shipped code does today. The snippets came from the external
> `abbey_wdbx_sea_zig` teaching repository and are retained as historical
> reference material; ABI does not depend on that repository being present or
> buildable. Their value is the clarity of the type and algorithm designs. Nothing
> here asserts performance, accuracy, retrieval-quality, or production-readiness
> numbers. ABI's own SEA surface (`crates/abi-sea/`) is a different current
> implementation; the relationship is mapped in §7. See also
...
## 2. `MemoryKind` — typed memory taxonomy (Current in ABI Rust)
...
## 3. `Authority` — provenance ladder with trust mapping (Current in ABI Rust)
...
## 7. Cross-reference: ABI SEA today

ABI has a SEA surface in the Rust workspace. The reference repository remains
historical input; the table below maps its concepts to the implementation that
is actually compiled and tested today.

| Aspect | `abbey_wdbx_sea_zig` (this extract — Proposed/Reference) | ABI Rust (`crates/abi-sea/` — Current) |
| --- | --- | --- |
| Status | Reference design whose core concepts informed ABI | `types.rs`, `scorer.rs`, `evidence.rs`, `learn_loop.rs`, and `query_plan.rs`; crate-tested |
| Retrieval scoring | 8 weighted signals, task-aware weight deltas, clamp+combine | 8-signal `sea_score()` weighted combiner + `adjust_weights_for_task` for code repair, project recall, and benchmark review |
| Selection | Budgeted greedy pick with token / record / **cluster-diversity** budgets | `select_sea_candidates` with token, record-count, and per-cluster budgets plus the documented high-score escape hatch |
...
| Typed memory | `MemoryKind` (9) + `Authority` (5) drive scoring | `MemoryKind` (9 variants) + `Authority` (5 rungs with `score()` trust mapping) drive scoring — both exported from `types.rs` |
| Loop | Score → select → context-pack → answer | `gather_evidence_with_plan` → `augment_prompt` → local completion → persisted adaptive-router update; selected evidence is rendered as bounded context |
...
2026-07-06 and were subsequently ported to `crates/abi-sea/src/types.rs` and
`crates/abi-sea/src/scorer.rs`; they pass `./tools/check.sh`.
...
- **Production-grade learned compression codec** — ABI has int8 quantization, order-0
  Huffman, and a reference autoencoder (`../wdbx/crates/abi-wdbx/src/neural_compress.rs`); no SOTA/production-scale
...
(`crates/abi-sea/`, `../wdbx/crates/abi-wdbx/`, `tests/golden/`) and downgrade anything
