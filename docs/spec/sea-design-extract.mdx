...
- **Production-grade learned compression codec** — ABI has int8 quantization, order-0
  Huffman, and a reference autoencoder (`crates/abi-wdbx/src/neural_compress.rs`); no SOTA/production-scale
...
(`crates/abi-sea/`, `crates/abi-wdbx/`, `tests/golden/`) and downgrade anything
