...
| --- | --- | --- |
| Status | Reference design; repo does not compile on current toolchain | Real code behind default-on `-Dfeat-sea`; `mod.zig`/`evidence.zig`/`learn_loop.zig`; contract-tested |
