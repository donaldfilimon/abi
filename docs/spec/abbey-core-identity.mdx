...
repository-evidence mapping (what the current Rust implementation actually proves).
Executable mirror: `crates/abi-ai/src/identity.rs`. Source, tests, and
...
The current Rust router implements a bounded subset: deterministic keyword
...
Pure `abi_ai::complete` is store-independent. The `abi complete` and
`abi agent train` CLI paths and MCP `ai_complete`, `ai_train`, and `ai_learn`
attempt the configured WDBX store, or `$HOME/.abi/wdbx` by default. The
top-level `abi train` command does not open WDBX. Operators opt out with
`ABI_WDBX_PERSIST=0` or
`ABI_WDBX_PATH=:memory:`. The completion metadata record stores derived vector
IDs, profile/model labels, audit/provenance fields, and byte counts; it
intentionally omits raw prompt and response text. Its stable
`persistence=explicit_store_result` value is a compatibility label, not a
current public request field. This is a bounded implementation of minimization,
not a claim of complete sensitive-data classification, encryption, deletion
workflows, RBAC, or regulatory compliance.
...
This is currently a **Proposed runtime capability** in the ABI Rust repository.
...
| --- | --- | --- |
| Identity and operating contract | **Current** | `crates/abi-ai/src/identity.rs`; this document; crate tests. |
| Abbey primary / Aviva direct / ABI orchestration roles | **Current** | `router.rs`, `incremental.rs`, neutral/direct/governance routing tests. Responses are local deterministic templates. |
...
| WDBX context and memory | **Partial** | Store-independent pure completion plus configured/default CLI/MCP persistence attempts, explicit opt-out, minimal provenance metadata, vectors/blocks, bounded SEA recall and authority weighting. No perfect recall/truth or full consent-management UI. |
| Accessibility adaptation | **Proposed** | Normative contract only; no verified multi-depth or assistive runtime. |
| Visual generation | **Proposed** | No image-generation model is embedded in the ABI Rust runtime. External tools may be used only when actually connected and disclosed. |
