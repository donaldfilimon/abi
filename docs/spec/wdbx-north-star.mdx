...
| Zero-copy memory access | **Partial** | `Store.getVector` returns a borrowed slice aliasing the index's contiguous backing buffer — no allocation/copy — proven by pointer-aliasing tests (including after `search` returns). HNSW distance already borrows during search; CLI/JSON query reports `dims` + `vector_view=borrowed` from `getVector` without copying vector bytes. Gap: `SearchResult`/`RankedNode` still carry ids/scores only — no owned result struct that threads borrowed vector slices through the full hybrid ranker return path. |
