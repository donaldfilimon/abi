...
WDBX Runtime
 ...
 ├─ Compute Layer       (Partial — CPU SIMD + macOS Metal fused cosine/dot/L2 + demo softmax + CPU fallback + CPU/GPU parity; ANE detection; TPU remote-dispatch transport)
...
| GPU-accelerated retrieval | **Partial** | HNSW distance routes through `gpu.vectorOps().cosineSimilarity()`; on macOS with `feat-gpu` and initialized Metal, that path uses linked `dot` / `squaredL2` / fused `cosine_parts_kernel` (`metal_shared.zig`) plus multi-pass `reduce_sum_kernel` (256-wide threadgroups until one scalar), else deterministic vectorized CPU; `vectorOps.softmax` adds a demo-grade Metal map+norm path (host-side max/partition-sum) with CPU/GPU parity; `compute.zig` adds a CPU/GPU/NPU/TPU backend selector; host-matched SIMD width via `std.simd.suggestVectorLength`; a CPU/GPU parity test validates the active backend against a scalar reference (`gpu/vector_ops.zig`). Gap: broader kernels and CUDA/Vulkan native dispatch are not linked; ANE execution remains out of scope. |
