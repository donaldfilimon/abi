...
- Authentication and security infrastructure (16 modules)
...
The framework compiles and passes all tests on Zig 0.16. Known remaining items:

- GPU/database backend source files have 37+ Zig 0.16 API changes needed
  (`*const DynLib`, stale struct fields, extern enum tag width)
- These compile through the named `abi` module but cannot be registered in
  `feature_test_root.zig` for direct testing
- Tracked for a dedicated migration pass
...
- **Complete Zig 0.16 backend migration** -- Resolve remaining 37+ API changes
  in GPU and database backend files
