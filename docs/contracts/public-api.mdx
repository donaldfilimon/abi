...
`abi auth` persists provider credentials as local plaintext JSON (resolved from `ABI_CREDENTIALS_PATH`, `XDG_CONFIG_HOME/abi/`, or `~/.abi/` as fallback). On POSIX-capable targets, the credential directory is created or repaired as owner-only (`0700`) and the credential file is opened/truncated with owner-only permissions (`0600`) before secret bytes are written; existing permissive files are tightened before overwrite. On POSIX TTYs, `auth signin` disables terminal echo while reading secrets and restores it afterward; Windows has no echo-suppress or ACL/keychain path yet (disclosed gap). This is local filesystem + TTY hardening, not encryption, OS keychain storage, RBAC, or a regulatory-compliance claim.
