...
The GPU feature has a stable runtime contract across real and disabled builds: all backends are listed, `detectBackend()` returns a safe backend status, vector operations use linked Metal `dot` / `squaredL2` / fused `cosine_parts` kernels plus multi-pass `reduce_sum_kernel` / `reduce_max_kernel` / `reduce_min_kernel`, elementwise `add` / `sub` / `max` / `min` / `div` via `compute_api`, unary `scale` / `relu` / `abs` / `negate` (`scale_kernel` / `relu_kernel` / `abs_kernel` / `negate_kernel`), elementwise `mul` via existing `dot_kernel` / `dot_pipeline` (`result[id]=a[id]*b[id]`; no separate `mul_kernel`), and demo-grade `softmax` (`softmax_kernel` + `softmax_norm_kernel`, on-GPU reduce_max when available, host partition-sum) on macOS when native kernels initialize (else deterministic vectorized CPU fallback), and `backendStatusReport()` explains whether Metal/native kernels are active or falling back. This does not prove CUDA/Vulkan/ANE dispatch or GPU speedup figures.
