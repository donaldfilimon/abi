...
fn runSingleBenchmark(
    ...
    var build_timer = abi.foundation.time.Timer.start() catch return error.TimerFailed;
    ...
    var search_timer = abi.foundation.time.Timer.start() catch return error.TimerFailed;

    for (queries, 0..) |query, qi| {
        var query_timer = abi.foundation.time.Timer.start() catch return error.TimerFailed;
